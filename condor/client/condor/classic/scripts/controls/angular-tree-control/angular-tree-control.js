                ...
                controller: ['$scope', function($scope) {
                    ...
                    $scope.headClass = function(node) {
                        ...
                        if (isSelectedNode(node)) {
                            injectSelectionClass = liSelectionClass ? " " + liSelectionClass : " ";

                            if ($scope.options.isSpecificNode(node)) {
                                injectSelectionClass = " " + classIfDefined($scope.options.injectClasses.iSpecific, false);
                            }
                        }
                    ...
                    $scope.ngClassIfDefined = function(ngClass) {
                        if (ngClass) {
                            return $scope.$parent.$eval(ngClass);
                        }
                        return '';
                    };

                    $scope.ulClass = $scope.options.injectClasses.ul ? $scope.$eval($scope.options.injectClasses.ul) : '';

                    //tree template
                    var orderBy = $scope.orderBy ? ' | orderBy:orderBy:reverseOrder' : '';
                    var template =
                        '<ul  ng-class="options.injectClasses.ul">' +
