angular.module('inprotech.configuration.general.validcombination')
    .controller('ValidSubTypeController', ValidSubTypeController);



function ValidSubTypeController($scope, kendoGridBuilder, validCombinationService, validCombinationMaintenanceService, BulkMenuOperations) {
    'use strict';
    ...
    function isResetDisabled() {
        return $scope.vm.form.$valid && $scope.vm.selectedSearchOption.type === 'default' && ($scope.vm.searchCriteria.jurisdictions === null || $scope.vm.searchCriteria.jurisdictions.length === 0) && (angular.equals($scope.vm.searchCriteria.propertyType, {}) || $scope.vm.searchCriteria.propertyType === null) && (angular.equals($scope.vm.searchCriteria.caseType, {}) || $scope.vm.searchCriteria.caseType === null) && (angular.equals($scope.vm.searchCriteria.caseCategory, {}) || $scope.vm.searchCriteria.caseCategory === null) && (angular.equals($scope.vm.searchCriteria.subType, {}) || $scope.vm.searchCriteria.subType === null) && vc.gridOptions.data().length === 0;
    }
    ...
    function buildGridOptions() {
        ...
        return kendoGridBuilder.buildOptions($scope, {
            ...
            columns: [{
                ...
                headerTemplate: '<div data-bulk-actions-menu data-items="vc.gridOptions.data()" data-actions="vc.actions" data-context="subtype" data-on-clear="vc.clearSelection();" is-full-selection-possible="false" data-on-select-this-page="vc.selectPage(val)" data-initialised="vc.menuInitialised()"></div>'
            }, {
                title: 'Case Type',
                field: 'caseType',
                width: '15%',
    ...
    function buildQuery(criteria) {
        return {
            ...
            subType: criteria.subType ? criteria.subType.code : null,
            caseCategory: criteria.caseCategory ? criteria.caseCategory.code : null
        };
    }
