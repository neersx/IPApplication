angular.module('inprotech.configuration.general.validcombination')
    .controller('validCategoryMaintenanceController', function($scope, selectedCaseType, validCombinationConfig, validCombinationService, $http) {
        'use strict';
        ...
        ctrl.allJurisdictionsMultiClass = allJurisdictionsMultiClass;

        getMultiClassApplicationCountries();

        function getMultiClassApplicationCountries() {
            return $http.get('api/picklists/caseCategories/multiclasscountries')
                .then(function(response) {
                    ctrl.multiClassJurisdictions = response.data;
                });
        }

        function allJurisdictionsMultiClass() {
            if($scope.model) {
                var jurisdictions = _.pluck($scope.model.jurisdictions, 'key');
                var includes = jurisdictions.every(function(j) {
                    if(ctrl.multiClassJurisdictions)
                    return ctrl.multiClassJurisdictions.includes(j);
                });
                if (includes) {
                    $scope.model.multiClassPropertyApp = false;
                }
                return includes;
            }
        }
        ...
        function populateSearchCriteria() {
            if ($scope.state === 'adding') {
                ...
                if (!angular.equals($scope.searchCriteria.caseCategory, {}) && $scope.searchCriteria.caseCategory) {
                    ...
                    onCaseCategorySelectionChanged(false);
                    $scope.model.prepopulated = true;
                }
            }
            selectedCaseType.set($scope.model.caseType);
        ...
        function onCaseCategorySelectionChanged(validDescription) {
            if (angular.isDefined($scope.maintenance) && $scope.maintenance.caseCategory) {
                $scope.maintenance.caseCategory.$setValidity('invalidcombination', true);
            }
            if (!validDescription.$dirty && $scope.model.caseCategory) {
                $scope.model.validDescription = $scope.model.caseCategory.value;
            }
        }
        ...
        function disableCategoryPicklist() {
            ...
            if (disableCategory && picklistHasValue($scope.model.caseCategory)) {
                ...
                $scope.model.validDescription = null;
