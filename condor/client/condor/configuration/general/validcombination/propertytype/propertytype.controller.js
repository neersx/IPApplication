angular.module('inprotech.configuration.general.validcombination')
    .controller('ValidPropertyTypeController', ValidPropertyTypeController);


function ValidPropertyTypeController($scope, kendoGridBuilder, validCombinationService, validCombinationMaintenanceService, BulkMenuOperations) {
    'use strict';

    var vc = this;
    vc.search = search;
    vc.context = 'propertyType';
    ...
    function isResetDisabled() {
        return $scope.vm.form.$valid && $scope.vm.selectedSearchOption.type === 'default' && ($scope.vm.searchCriteria.jurisdictions === null || $scope.vm.searchCriteria.jurisdictions.length === 0) && (angular.equals($scope.vm.searchCriteria.propertyType, {}) || $scope.vm.searchCriteria.propertyType === null) && vc.gridOptions.data().length === 0;
    }
    ...
    function buildGridOptions() {
        ...
        return kendoGridBuilder.buildOptions($scope, {
            ...
            columns: [{
                ...
                headerTemplate: '<div data-bulk-actions-menu data-items="vc.gridOptions.data()" data-actions="vc.actions" data-context="propertyType" data-on-clear="vc.clearSelection();" is-full-selection-possible="false" data-on-select-this-page="vc.selectPage(val)" data-initialised="vc.menuInitialised()"></div>'
            }, {
                title: 'Jurisdiction',
                field: 'country',
                width: '20%',
