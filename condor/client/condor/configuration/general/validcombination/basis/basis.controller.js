angular.module('inprotech.configuration.general.validcombination')
    .controller('ValidBasisController', ValidBasisController);



function ValidBasisController($scope, kendoGridBuilder, validCombinationService, validCombinationMaintenanceService, BulkMenuOperations) {
    'use strict';
    ...
    function isResetDisabled() {
        return $scope.vm.form.$valid && $scope.vm.selectedSearchOption.type === 'default' && ($scope.vm.searchCriteria.jurisdictions === null || $scope.vm.searchCriteria.jurisdictions.length === 0) && (angular.equals($scope.vm.searchCriteria.propertyType, {}) || $scope.vm.searchCriteria.propertyType === null) && (angular.equals($scope.vm.searchCriteria.caseType, {}) || $scope.vm.searchCriteria.caseType === null) && (angular.equals($scope.vm.searchCriteria.caseCategory, {}) || $scope.vm.searchCriteria.caseCategory === null) && (angular.equals($scope.vm.searchCriteria.basis, {}) || $scope.vm.searchCriteria.basis === null) && vc.gridOptions.data().length === 0;
    }
    ...
    function buildGridOptions() {
        ...
        return kendoGridBuilder.buildOptions($scope, {
            ...
            columns: [{
                ...
                headerTemplate: '<div data-bulk-actions-menu data-items="vc.gridOptions.data()" data-actions="vc.actions" data-context="basis" data-on-clear="vc.clearSelection();" is-full-selection-possible="false" data-on-select-this-page="vc.selectPage(val)" data-initialised="vc.menuInitialised()"></div>'
            }, {
                title: 'Case Type',
                field: 'caseType',
                width: '15%',
    ...
    function buildQuery(criteria) {
        return {
            ...
            basis: criteria.basis ? criteria.basis.code : null,
