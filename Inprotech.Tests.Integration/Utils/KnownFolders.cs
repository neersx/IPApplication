...
namespace Inprotech.Tests.Integration.Utils
{
    ...
    {
        static readonly string[] KnownFolderGuids =
        {
        ...
        static string GetPath(KnownFolder knownFolder, KnownFolderFlags flags,
                              bool defaultUser)
        {
            IntPtr outPath;
            var result = SHGetKnownFolderPath(new Guid(KnownFolderGuids[(int) knownFolder]), (uint) flags, new IntPtr(defaultUser ? -1 : 0), out outPath);
