...
namespace Inprotech.Tests.Integration.EndToEnd.Portfolio.Cases
{
    ...
    [TestFixture]
    public class CaseViewEventsAttachments : IntegrationTest
    {
        [SetUp]
        public void Setup()
        {
        ...
        public void CaseViewAttachmentsActionEventsAddEdit(BrowserType browserType)
        {
            var data = new CaseDetailsAttachmentSetup().AttachmentSetup(true);
            ...
            var eventTopic = new CaseEventsTopic(driver);
            Assert.AreEqual(1, eventTopic.DueDatesGrid.AttachmentIcons.Count, "One attachment icon is displayed");
            eventTopic.DueDatesGrid.AttachmentIcons.First().Click();
