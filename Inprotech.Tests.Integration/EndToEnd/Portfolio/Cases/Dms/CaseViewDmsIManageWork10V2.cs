...
namespace Inprotech.Tests.Integration.EndToEnd.Portfolio.Cases.Dms
{
    ...
    [TestFixture]
    public class CaseViewDmsIManageWork10V2 : IntegrationTest
    {
        ...
        public void DisplayDmsSection(BrowserType browserType)
        {
            ...
            Assert.NotNull(dms.LoginOauth2Alert);
            dms.LoginOauth2Alert.FindElement(By.TagName("a")).WithJs().Click();
            driver.Wait().ForTrue(() => dms.DirectoryTreeView.Folders.Count != 0, 20000);
            dms.DirectoryTreeView.Folders[0].Click();
            ...
            dms.Documents.Cell(0, 5).ClickWithTimeout();

            var detail = dms.Documents.DocumentDetail(0);
            Assert.AreEqual("comment", detail.Comments.Value());
            Assert.True(detail.RelatedDocuments.Count > 0);

            Assert.False(string.IsNullOrWhiteSpace(detail.RelatedDocuments[0].FindElements(By.TagName("td"))[3].Text));
            Assert.False(string.IsNullOrWhiteSpace(detail.RelatedDocuments[0].FindElements(By.TagName("td"))[4].Text));
            Assert.False(string.IsNullOrWhiteSpace(detail.RelatedDocuments[0].FindElements(By.TagName("td"))[5].Text));
        ...
        Case Setup()
        {
            ...
            DbSetup.Do(db =>
            {
                ...
                new DocumentManagementDbSetup().Setup(IntegrationType.Work10V2);
