...
using Extended = Inprotech.Integration.Schedules.Extensions.Uspto.Tsdr;
...
namespace Inprotech.Tests.Integration.EndToEnd.Integration.PtoAccess.Uspto
{
    ...
    internal class TsdrSchedule : IntegrationTest
    {
        ...
        public void NewScheduleTests(BrowserType browserType)
        {
            ...
            using (var dbSetup = new TsdrSettingDbSetup())
            {
            ...
            driver.With<NewTsdrSchedulePageObject>((page, popup) =>
            {
                page.ScheduleName.Input.SendKeys(name);

                page.DataSourceType.Input.SelectByText("USPTO TSDR");
            ...
            IntegrationDbSetup.Do(x =>
            {
                ...
                var tsdrScheduleDetails = schedule.GetExtendedSettings<Extended.TsdrSchedule>();
                ...
                Assert.NotNull(tsdrScheduleDetails.RunAsUserId, "Should save schedule run as user id.");

                Assert.NotNull(tsdrScheduleDetails.RunAsUserName, "Should save schedule run as user name.");

                Assert.NotNull(tsdrScheduleDetails.SavedQueryId, "Should save schedule with saved query id.");

                Assert.NotNull(tsdrScheduleDetails.SavedQueryName, "Should save schedule with saved query name.");
            });
            ...
            driver.With<SchedulesPageObject>(page =>
            {
                ...
                Assert.AreEqual(1, schedules.Length, "Should list the only created Tsdr schedule.");

                Assert.AreEqual("USPTO TSDR", schedules[0].Source, "Should show that its Datasource is 'USPTO TSDR'");
