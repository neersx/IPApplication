...
namespace Inprotech.Tests.Integration.IntegrationTests.Accounting.Time
{
    ...
    public class EditingPostedContinuedTime : EditingPostedTime
    {
        TestUser _user;
        Diary _diary;
        Name _entityName;
        ...
        [Test]
        public override void RecordsCorrectlyUpdatedOnIncreasingDuration()
        {
        ...
        [Test]
        public override void RecordsCorrectlyUpdatedOnDecreasingDuration()
        {
            var transNo = PostedTimeTestHelper.PostEntry(_entityName, _diary, _user);
            var editRequest = PostedTimeTestHelper.ChangeDurationBy(PostedTimeTestHelper.GetRecordableTime(_diary, _entityName), -0.5);
            ...
            DbSetup.Do(x =>
            {
                ...
                var wips = x.DbContext.Set<WorkInProgress>().Where(_ => _.TransactionId == updatedDiary.TransactionId && _.EntityId == updatedDiary.WipEntityId).ToList();
                PostedTimeTestHelper.AssertWipDetails(wips, new DateTime(1899, 1, 1, 1, 30, 0), 15, 10, 150, 300, 225, -30, -22.50, "Edited posted entry(Subtraction of 1 hr)");

                var history = x.DbContext.Set<WorkHistory>().Where(_ => _.TransactionId == updatedDiary.TransactionId && _.EntityId == updatedDiary.WipEntityId && _.HistoryLineNo == 2).ToList();
                PostedTimeTestHelper.AssertWorkflowHistory(history, -5, -75, 7.50, MovementClass.AdjustDown, MovementClass.AdjustUp, "Edited posted entry(Subtraction of 1 hr)");
            });
        }

        [Test]
        public override void RecordsCorrectlyUpdatedOnChangeOfNarrative()
        {
            var transNo = PostedTimeTestHelper.PostEntry(_entityName, _diary, _user);
        ...
        [Test]
        public override void RecordsCaseWipTransferCorrectly()
        {
        ...
        [Test]
        public override void RecordsActivityWipTransferCorrectly()
        {
            PostedTimeTestHelper.PostEntry(_entityName, _diary, _user);
            var editRequest = PostedTimeTestHelper.GetRecordableTime(_diary, _entityName);
        ...
        [Test]
        public override void RecordsCaseAndActivityWipTransferCorrectly()
        {
            PostedTimeTestHelper.PostEntry(_entityName, _diary, _user);
            var editRequest = PostedTimeTestHelper.GetRecordableTime(_diary, _entityName);
            editRequest.Activity = _newActivity.WipCode;
        ...
        void SetupData()
        {
            DbSetup.Do(x =>
            {
                ...
                var parentDiary = new DiaryBuilder(x.DbContext).Create(_user.NameId, 10, startTime, @case.Id, null, activity.WipCode, "short-narrative", "note1", null, 300, 150, 30);
                
                _diary = new DiaryBuilder(x.DbContext).Create(_user.NameId, 11, startTime, @case.Id, null, activity.WipCode, "short-narrative", "note1", null, 300, 150, 30, false, parentDiary.EntryNo);
                _diary.TotalUnits = 20;
                _diary.FinishTime = startTime.AddHours(1);
                _diary.TotalTime = new DateTime(1899, 1, 1, 1, 0, 0);
                _diary.TimeCarriedForward = new DateTime(1899, 1, 1, 1, 0, 0);
