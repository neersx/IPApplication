...
{
    public class VerifyReaderWriterAccessToIntegrationDatabase : ISetupAction
    {
        public string Description => "Verify reader/writer access to Integration database";
        ...
        public void Run(IDictionary<string, object> context, IEventStream eventStream)
        {
            ...
            if (builder.IntegratedSecurity)
            {
                ...
                DatabaseUtility.GrantReaderWriterAccess(integrationAdministrationConnectionString, serviceUser.ToCanonicalUserName());
            }
            else
            {
                eventStream.PublishInformation($"Integrated Security=false. user={builder.UserID}");
                DatabaseUtility.GrantReaderWriterAccess(integrationAdministrationConnectionString, builder.UserID);
            }
        }
    }
}