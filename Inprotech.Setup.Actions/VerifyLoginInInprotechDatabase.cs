...
{
    public class VerifyLoginInInprotechDatabase : ISetupAction
    {
        public string Description => "Verify login in Inprotech database";
        ...
        public void Run(IDictionary<string, object> context, IEventStream eventStream)
        {
            ...
            if(builder.IntegratedSecurity)
            {
                ...
                DatabaseUtility.EnsureWindowsUserLogin(inprotechAdministrationConnectionString, serviceUser.ToCanonicalUserName());
            }
            else
            {
                eventStream.PublishInformation($"Integrated Security=false. user={builder.UserID}");
                DatabaseUtility.EnsureSqlUserLogin(inprotechAdministrationConnectionString, builder.UserID);
            }
        }        
    }
}