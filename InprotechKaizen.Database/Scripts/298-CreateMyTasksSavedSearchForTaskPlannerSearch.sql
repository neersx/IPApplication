if not exists (select * from QUERYFILTER where PROCEDURENAME = 'ipw_TaskPlanner' and FILTERID = -27)
begin
...
<Search>
   <Filtering>
	...
	values (-27, 'ipw_TaskPlanner', @xmlFilter) 
...
if not exists (select * from QUERY where QUERYID = -27)
begin
...
-- Disable the insert audit trigger if exists and is an instead of trigger and not disabled.
	...
	values (-27, 970, null, 'My Tasks', 'Reminders, Due Dates and Ad Hocs for the next two weeks where I am the Reminder Recipient, the Staff or Signatory on the case, or have Due Date Responsibility.', -27, @nNewPresentationId)
...
UPDATE QUERY SET QueryName = 'My Tasks' WHERE QueryId = -27
go

declare @nPresentationId int
select @nPresentationId = PRESENTATIONID from QUERY where CONTEXTID = 970 and FILTERID = -27
...
AND DI.PROCEDURENAME = N'ipw_TaskPlanner'
	If NOT exists(	SELECT * FROM QUERYCONTENT WHERE COLUMNID = cast(@nColumnId as nvarchar) AND PRESENTATIONID = @nPresentationId)
        	BEGIN
         	 PRINT '**** DR-62615 Adding data QUERYCONTENT for COLUMNID = ' + cast(@nColumnId as nvarchar) + ' AND PRESENTATIONID = ' + cast(@nPresentationId as nvarchar)
		 INSERT INTO QUERYCONTENT (PRESENTATIONID, COLUMNID, DISPLAYSEQUENCE, SORTORDER, SORTDIRECTION, CONTEXTID)
		 SELECT DISTINCT @nPresentationId, cast(@nColumnId as nvarchar), 2, 2, NULL, 970
		 FROM (select 1 as txt) TMP
		 left join QUERYPRESENTATION P on (P.CONTEXTID = 970)
		 where ISNULL(P.ISDEFAULT, 0) = 0
		 or ISNULL(P.ISPROTECT, 0) = 0
...
AND DI.PROCEDURENAME = N'ipw_TaskPlanner'
	If NOT exists(	SELECT * FROM QUERYCONTENT WHERE COLUMNID = cast(@nColumnId as nvarchar) AND PRESENTATIONID = @nPresentationId)
        	BEGIN
         	 PRINT '**** DR-62615 Adding data QUERYCONTENT for COLUMNID = ' + cast(@nColumnId as nvarchar) + ' AND PRESENTATIONID = ' + cast(@nPresentationId as nvarchar)
		 INSERT INTO QUERYCONTENT (PRESENTATIONID, COLUMNID, DISPLAYSEQUENCE, SORTORDER, SORTDIRECTION, CONTEXTID)
		 SELECT DISTINCT @nPresentationId, cast(@nColumnId as nvarchar), 3, 1, NULL, 970
		 FROM (select 1 as txt) TMP
		 left join QUERYPRESENTATION P on (P.CONTEXTID = 970)
		 where ISNULL(P.ISDEFAULT, 0) = 0
		 or ISNULL(P.ISPROTECT, 0) = 0
