...
as RETURN
	...
	(
		...
		-- user.
		-------------------------------------------
		JOIN ROWACCESSDETAIL RAD ON (RAD.ACCESSNAME = UA.ACCESSNAME AND RAD.RECORDTYPE = 'C' )
		JOIN CASES CO ON (( CO.OFFICEID = RAD.OFFICE OR RAD.OFFICE IS NULL ) AND ( CO.CASETYPE = RAD.CASETYPE     OR RAD.CASETYPE     IS NULL ) AND ( CO.PROPERTYTYPE = RAD.PROPERTYTYPE OR RAD.PROPERTYTYPE IS NULL ) )
