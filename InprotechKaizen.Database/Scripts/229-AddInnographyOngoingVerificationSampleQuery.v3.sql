...
<csw_ListCase>
	<FilterCriteriaGroup>
		<FilterCriteria ID="1">
			...
			<Event Operator="7" IsRenewalsOnly="1" IsNonRenewalsOnly="1" ByDueDate="0" ByEventDate="1">
                <EventKey>-14</EventKey>
                <Period>
                    <Type>D</Type>
                    <Quantity>-7</Quantity>
                </Period>
...
if not exists (select * from QUERYFILTER where PROCEDURENAME = 'csw_ListCase' and FILTERID = -26)
begin
	...
	values (-26, 'csw_ListCase', @xmlFilterWithMark) 
...
else if exists(select * from QUERYFILTER where PROCEDURENAME = 'csw_ListCase' and FILTERID = -26 and cast(XMLFILTERCRITERIA as nvarchar(max)) = @xmlFilterWithoutMark)
begin
...
if not exists (select * from QUERY where QUERYID = -26)
begin

	insert QUERY (QUERYID, CONTEXTID, IDENTITYID, QUERYNAME, DESCRIPTION, FILTERID)
	values (-26, 2, null, 'Innography Ongoing Verification - Sample', 'A sample query returning cases for ongoing data verification with Innography Integration . The system will use this query to determine the list of eligible cases.', -26)
...
UPDATE QUERY SET QueryName = 'Innography Initial Matching - Sample' WHERE QueryId = -25 and QueryName <> 'Innography Initial Matching - Sample'
GO
