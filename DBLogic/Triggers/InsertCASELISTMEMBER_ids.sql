if exists (select * from sysobjects where type='TR' and name = 'InsertCASELISTMEMBER_ids')
begin
	PRINT 'Refreshing trigger InsertCASELISTMEMBER_ids...'
	DROP TRIGGER InsertCASELISTMEMBER_ids
end
go

Create trigger InsertCASELISTMEMBER_ids on CASELISTMEMBER for INSERT NOT FOR REPLICATION as
Begin
-- TRIGGER:	InsertCASELISTMEMBER_ids    
-- VERSION:	3
-- DESCRIPTION:	
...
--					linked to the Prior Art as a related Case.
	...
	-------------------------------------------------------
	-- Cases may be added to a Case List. If the Case List
	-- is associated with a Search Result (Source Document 
	-- or Prior Art) then the Case should be linked to the 
	-- Prior Art.
	-------------------------------------------------------
	Insert into CASESEARCHRESULT(CASELISTPRIORARTID, CASEID, PRIORARTID, STATUS, UPDATEDDATE,ISCASERELATIONSHIP)
	Select distinct L.CASELISTPRIORARTID, i.CASEID, L.PRIORARTID, CASE WHEN(CS1.STATUS=-999999999) THEN NULL ELSE CS1.STATUS END, getdate(),0
	From inserted i
	join CASELISTSEARCHRESULT L
				on (L.CASELISTNO=i.CASELISTNO)
	join SEARCHRESULTS S	on (S.PRIORARTID=L.PRIORARTID)
	...
	left join (	select  CASEID, PRIORARTID, max(isnull(STATUS,-999999999)) as STATUS
			...
			group by CASEID, PRIORARTID) CS1
				...
				and CS1.PRIORARTID=L.PRIORARTID)
			---------------------------------------------
			-- Do not insert the row if it already exists
			---------------------------------------------
	left join CASESEARCHRESULT CS2 
				on (CS2.CASELISTPRIORARTID=L.CASELISTPRIORARTID
				and CS2.CASEID=i.CASEID
				and CS2.PRIORARTID=L.PRIORARTID
	...
	-------------------------------------------------------
	-- Case added to a CaseList associated with Source Doc.
	-- A Source Document may bundle together any number of
	-- cited Search Results (prior art). When a Case is 
	-- is added to a CaseList that is  associated with a 
	...
	-------------------------------------------------------
	Insert into CASESEARCHRESULT(CASELISTPRIORARTID, CASEID, PRIORARTID, STATUS, UPDATEDDATE,ISCASERELATIONSHIP)
	Select distinct L.CASELISTPRIORARTID, i.CASEID, R.CITEDPRIORARTID, CASE WHEN(CS1.STATUS=-999999999) THEN NULL ELSE CS1.STATUS END, getdate(),0
	From inserted i
	join CASELISTSEARCHRESULT L
				on (L.CASELISTNO=i.CASELISTNO)
	join SEARCHRESULTS S	on (S.PRIORARTID=L.PRIORARTID)
