/******************************************************************************************************************/
/*** Create UpdateNAMEINSTRUCTIONS trigger									***/
/******************************************************************************************************************/     
if exists (select * from sysobjects where type='TR' and name = 'UpdateNAMEINSTRUCTIONS')
begin
 	PRINT 'Refreshing trigger UpdateNAMEINSTRUCTIONS...'
	DROP TRIGGER UpdateNAMEINSTRUCTIONS
end
else
	PRINT 'Creating trigger UpdateNAMEINSTRUCTIONS...'
	print ''
go
...
CREATE TRIGGER UpdateNAMEINSTRUCTIONS ON NAMEINSTRUCTIONS FOR UPDATE NOT FOR REPLICATION AS
-- TRIGGER:	UpdateNAMEINSTRUCTIONS    
-- VERSION:	1
-- DESCRIPTION:	If a NameInstructions row that is specific to a particular Case
--		is updated then a recalculation is required for Case level
...
--					donot have any data in COUNTRYCODE and PROPERTYTYPE
	...
	---------------------------------------------------
	-- Update of a Standing Instruction against either
