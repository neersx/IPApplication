if exists (select * from sysobjects where type='TR' and name = 'ti_CaseNameContact')
begin
	PRINT 'Refreshing trigger ti_CaseNameContact...'
	DROP TRIGGER ti_CaseNameContact
end
go

CREATE TRIGGER ti_CaseNameContact
ON CASENAME
FOR INSERT NOT FOR REPLICATION AS
-- TRIGGER:	ti_CaseNameContact  
-- VERSION:	7
...
--					trigger so that all rows in the 'inserted' table can be processed
...
Begin
	...
	Begin
		...
		-- associated with the Case and Names inserted
		---------------------------------------------
		Delete ACC
