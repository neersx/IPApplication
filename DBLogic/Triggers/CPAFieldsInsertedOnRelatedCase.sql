if exists (select * from sysobjects where type='TR' and name = 'CPAFieldsInsertedOnRelatedCase')
begin
	PRINT 'Refreshing trigger CPAFieldsInsertedOnRelatedCase...'
	DROP TRIGGER CPAFieldsInsertedOnRelatedCase
end
go
	
CREATE TRIGGER CPAFieldsInsertedOnRelatedCase ON RELATEDCASE
FOR INSERT NOT FOR REPLICATION AS
-- TRIGGER:	CPAFieldsInsertedOnRelatedCase    
-- VERSION:	3
-- DESCRIPTION:	
...
-- 31-AUG-2005	MF	SQA10874 3	Add new columns to CPAUPDATE and initialise
	...
	Select distinct i.CASEID, @nIdentityId, system_user, getdate(),'CPAFieldsInsertedOnRelatedCase',APP_NAME()
	from inserted i
