if exists (select * from sysobjects where type='TR' and name = 'UpdateCASEEVENT_Instruct')
begin
	PRINT 'Refreshing trigger UpdateCASEEVENT_Instruct...'
	DROP TRIGGER UpdateCASEEVENT_Instruct
end
go
	
CREATE TRIGGER UpdateCASEEVENT_Instruct ON CASEEVENT
FOR UPDATE NOT FOR REPLICATION AS
-- TRIGGER:	UpdateCASEEVENT_Instruct    
-- VERSION:	6
...
--					Also reverse SQA17490 as CASEEVENT audit triggers are an "instead of" trigger and so this trigger
--					will only fire once.
-- 16 Mar 2010	MF	RFC9031	4	Revisit.  Need to test that IND.ACTION is null.
-- 01 Jun 2010	MF	RFC9415	5	Change procedure so that CASEINSTRUCTALLOWED is only inserted by the
...
-- 29 Jul 2016	MF	64248	6	Don't fire the trigger when CASEEVENT row is having is LOGDATETIMESTAMP column changed.
...
Begin
	...
	-------------------------------------------------------------
	-- Delete CASEINSTRUCTALLOWED for any definitions associated
	-- with the EventNo being updated either as :
	--	INSTRUCTIONDEFINITION.DUEEVENTNO;
	--	INSTRUCTIONDEFINITION.PREREQUISITEEVENTNO; or
	--	INSTRUCTIONRESPONSE.FIREEVENTNO

	-- This is a heavy handed approach however the following step 
	-- will reinsert the CASEINSTRUCTALLOWED row if it should not 
	-- have been deleted.
	-------------------------------------------------------------
	Delete CI
	from CASEINSTRUCTALLOWED CI
	join inserted i			on (i.CASEID=CI.CASEID)
	join INSTRUCTIONDEFINITION D	on (D.DEFINITIONID=CI.DEFINITIONID)
	left join INSTRUCTIONRESPONSE R	on (R.DEFINITIONID=CI.DEFINITIONID
					and R.FIREEVENTNO=i.EVENTNO)
	where i.EVENTNO in (D.DUEEVENTNO,D.PREREQUISITEEVENTNO,R.FIREEVENTNO)
	...
	or exists(select 1
		  ...
		  join inserted i		   on (i.CASEID=d.CASEID
						   ...
						   and i.CYCLE=d.CYCLE
						   and i.OCCURREDFLAG<>d.OCCURREDFLAG)
		  join INSTRUCTIONRESPONSE R	   on (R.FIREEVENTNO=d.EVENTNO)
		  left join CASEINSTRUCTALLOWED CI on (CI.CASEID=d.CASEID
						   and CI.DEFINITIONID=R.DEFINITIONID)
		  where d.OCCURREDFLAG=1
