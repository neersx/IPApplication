if exists (select * from sysobjects where type='TR' and name = 'DeleteCASEEVENT_Instruct')
begin
	PRINT 'Refreshing trigger DeleteCASEEVENT_Instruct...'
	DROP TRIGGER DeleteCASEEVENT_Instruct
end
go
	
CREATE TRIGGER DeleteCASEEVENT_Instruct ON CASEEVENT
FOR DELETE NOT FOR REPLICATION AS
-- TRIGGER:	DeleteCASEEVENT_Instruct    
-- VERSION:	4
...
-------------------------------------------------------------------
-- If an Event that caused the CASEINSTRUCTALLOWED to be inserted
-- is subsequently removed then delete the CASEINSTRUCTALLOWED row.
-- This is a heavy handed approach however the following step will 
-- reinsert the CASEINSTRUCTALLOWED row if it is still required.
-------------------------------------------------------------------
Delete CASEINSTRUCTALLOWED
from CASEINSTRUCTALLOWED CI
join deleted d			on (d.CASEID=CI.CASEID)
join INSTRUCTIONDEFINITION ID	on (ID.DEFINITIONID=CI.DEFINITIONID
				and(ID.DUEEVENTNO=d.EVENTNO
				or  ID.PREREQUISITEEVENTNO=d.EVENTNO))
...
Begin
	...
	from deleted d
	JOIN INSTRUCTIONDEFINITION IND	on (d.EVENTNO in (IND.PREREQUISITEEVENTNO,IND.DUEEVENTNO))
