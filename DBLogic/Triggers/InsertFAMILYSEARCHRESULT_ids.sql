if exists (select * from sysobjects where type='TR' and name = 'InsertFAMILYSEARCHRESULT_ids')
begin
	PRINT 'Refreshing trigger InsertFAMILYSEARCHRESULT_ids...'
	DROP TRIGGER InsertFAMILYSEARCHRESULT_ids
end
go

Create trigger InsertFAMILYSEARCHRESULT_ids on FAMILYSEARCHRESULT for INSERT NOT FOR REPLICATION as
Begin
-- TRIGGER:	InsertFAMILYSEARCHRESULT_ids    
-- VERSION:	3
-- DESCRIPTION:	
...
--					linked to the Prior Art as a related Case.
	...
	-------------------------------------------------------
	-- For each Case belonging to a Family that has been
	-- linked to a Search Result, we need to generate the
	-- link between the Case and the Search Result.
	...
	Select distinct i.FAMILYPRIORARTID, C.CASEID, i.PRIORARTID, CASE WHEN(CS1.STATUS=-999999999) THEN NULL ELSE CS1.STATUS END, getdate(),0
	From inserted i
	...
	left join CASESEARCHRESULT CS2 
				on (CS2.FAMILYPRIORARTID=i.FAMILYPRIORARTID
	...
	-------------------------------------------------------
	-- Family linked to Source Document
	...
	-------------------------------------------------------
	Insert into CASESEARCHRESULT(FAMILYPRIORARTID, CASEID, PRIORARTID, STATUS, UPDATEDDATE,ISCASERELATIONSHIP)
	Select distinct i.FAMILYPRIORARTID, C.CASEID, R.CITEDPRIORARTID, CASE WHEN(CS1.STATUS=-999999999) THEN NULL ELSE CS1.STATUS END, getdate(),0
	From inserted i
	join SEARCHRESULTS S	on (S.PRIORARTID=i.PRIORARTID)
			---------------------------------------------
