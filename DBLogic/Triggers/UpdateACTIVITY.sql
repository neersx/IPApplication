if exists (select * from sysobjects where type='TR' and name = 'UpdateACTIVITY')
begin
	PRINT 'Refreshing trigger UpdateACTIVITY...'
	DROP TRIGGER UpdateACTIVITY
end
go

CREATE TRIGGER UpdateACTIVITY on ACTIVITY for UPDATE NOT FOR REPLICATION as
...
begin
	 ...
	 /* NAME in relation to ACTIVITY ON CHILD UPDATE RESTRICT */
	 if update(RELATEDNAME)
		begin
		 ...
		 if @validcnt + @nullcnt != @numrows
			begin
			 select @errno  = 30007,
     			 @errmsg = 'Error %d Cannot UPDATE ACTIVITY because NAME does not exist.'
			 goto error
			end
		end
