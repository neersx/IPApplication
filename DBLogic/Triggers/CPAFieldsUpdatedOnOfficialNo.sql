if exists (select * from sysobjects where type='TR' and name = 'CPAFieldsUpdatedOnOfficialNo')
begin
	PRINT 'Refreshing trigger CPAFieldsUpdatedOnOfficialNo...'
	DROP TRIGGER CPAFieldsUpdatedOnOfficialNo
end
go
	
CREATE TRIGGER CPAFieldsUpdatedOnOfficialNo ON OFFICIALNUMBERS
FOR UPDATE NOT FOR REPLICATION AS
-- TRIGGER:	CPAFieldsUpdatedOnOfficialNo    
-- VERSION:	4
-- DESCRIPTION:	
...
Begin
	...
	Select distinct i.CASEID, @nIdentityId, system_user, getdate(),'CPAFieldsUpdatedOnOfficialNo',APP_NAME()
	from inserted i
