if exists (select * from sysobjects where type='TR' and name = 'UpdateRELATEDCASE_ids    ')
begin
	PRINT 'Refreshing trigger UpdateRELATEDCASE_ids    ...'
	DROP TRIGGER UpdateRELATEDCASE_ids    
end
go

Create trigger UpdateRELATEDCASE_ids     on RELATEDCASE for UPDATE NOT FOR REPLICATION as
-- TRIGGER:	UpdateRELATEDCASE_ids    
-- VERSION:	8
-- DESCRIPTION:	
...
-- 16 Jun 2015	MF	45361	6	Further rework to cater for extremely large volume of prior art distribution by
...
--					keys being inserted.
-- 24 Feb 2016	DL	57674	9	Remove launching of policing asynchronously as it can cause deadlock.  
--					Start policing Continuously if it has not started and submit policing request to this queue.
Begin

If update(RELATEDCASEID)
Begin
	...
	Begin
		...
		Begin	
			...
			--------------------------------------------------
			-- Police if the RELATEDCASEID or the RELATIONSHIP
			-- is being changed as this can impact the flow
			-- of prior art throughout the extended family.
			--------------------------------------------------
			insert into @tblPolicing(CASEID)
			select distinct i.CASEID
			from inserted i
			join deleted d	on (d.CASEID=i.CASEID
					and d.RELATIONSHIPNO=i.RELATIONSHIPNO
					and(d.RELATEDCASEID<>i.RELATEDCASEID or (d.RELATEDCASEID is null and i.RELATEDCASEID is not null) or (d.RELATIONSHIP<>i.RELATIONSHIP)))
