-----------------------------------------------------------------------------------------------------------------------------
-- Creation of util_DropTranslationTriggers
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[util_DropTranslationTriggers]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.util_DropTranslationTriggers.'
	drop procedure dbo.util_DropTranslationTriggers
	print '**** Creating procedure dbo.util_DropTranslationTriggers...'
	print ''
end
go
...
CREATE PROCEDURE dbo.util_DropTranslationTriggers
			@psTable	varchar(30)	-- Mandatory
	
AS

-- PROCEDURE :	util_DropTranslationTriggers
-- VERSION :	1
-- DESCRIPTION:	Base on util_GenerateTranslationTriggers, to create scripts that drop the triggers

-- MODIFICATION
-- Date		Who	Number	Version	Description
-- ----         ---	------	-------	-----------------------------------------------------------
-- 14 Feb 2008	SW		1	Procedure created
...
End
--------------------------------------------------------------------------------------
-- Dropping of the DELETE trigger
--------------------------------------------------------------------------------------

Insert into @tbResult(ResultString)
select
"-----------------------------------------------------------------------------------------------------------------------------
-- Dropping of trigger tD_"+@psTable+"_Translation
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where type='TR' and name = 'tD_"+@psTable+"_Translation')
begin
	PRINT 'Dropping trigger tD_"+@psTable+"_Translation...'
...
select
"-----------------------------------------------------------------------------------------------------------------------------
-- Dropping of trigger tI_"+@psTable+"_Translation
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where type='TR' and name = 'tI_"+@psTable+"_Translation')
begin
	PRINT 'Dropping trigger tI_"+@psTable+"_Translation...'
...
select
"-----------------------------------------------------------------------------------------------------------------------------
-- Dropping of trigger tU_"+@psTable+"_Translation
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where type='TR' and name = 'tU_"+@psTable+"_Translation')
begin
	PRINT 'Dropping trigger tU_"+@psTable+"_Translation...'
...
grant execute on dbo.util_DropTranslationTriggers  to public
go
