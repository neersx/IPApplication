-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ipw_ListCaseControlCriteriaTree
-----------------------------------------------------------------------------------------------------------------------------
If exists (select * from dbo.sysobjects where id = object_id(N'[dbo].ipw_ListCaseControlCriteriaTree') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
Begin
	Print '**** Drop Stored Procedure dbo.ipw_ListCaseControlCriteriaTree.'
	Drop procedure [dbo].ipw_ListCaseControlCriteriaTree
End
Print '**** Creating Stored Procedure dbo.ipw_ListCaseControlCriteriaTree...'
Print ''
GO
...
CREATE PROCEDURE dbo.ipw_ListCaseControlCriteriaTree
(
...
as
-- PROCEDURE:	ipw_ListCaseControlCriteriaTree
-- VERSION:	4
...
-- -----------	-------	-------	-------	----------------------------------------------- 
-- 17 Mar 2009	LP	RFC7208	1	Procedure created
-- 27 Aug 2009  LP      RFC7580 2       Removed debugging print statement.
-- 11 Sep 2009	LP	RFC8047	3	Return ProfileName column.	
-- 10 Apr 2017	MF	71141	4	Duplicate rows being returned because JOIN to CASECATEGORY needs to also include CASETYPE.
...
Begin
	...
	"                          and CC.CASETYPE     = C.CASETYPE)"+char(10)+
...
Grant execute on dbo.ipw_ListCaseControlCriteriaTree to public
GO
