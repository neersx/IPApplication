-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_COUNTRYGROUP
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_COUNTRYGROUP]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_COUNTRYGROUP.'
	drop procedure dbo.ip_cc_COUNTRYGROUP
	print '**** Creating procedure dbo.ip_cc_COUNTRYGROUP...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_COUNTRYGROUP
...
-- Prerequisite that the CCImport_COUNTRYGROUP table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_COUNTRYGROUP('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update COUNTRYGROUP
		set	DATECOMMENCED= I.DATECOMMENCED,
			DATECEASED= I.DATECEASED,
			ASSOCIATEMEMBER= I.ASSOCIATEMEMBER,
			DEFAULTFLAG= I.DEFAULTFLAG,
			PREVENTNATPHASE= I.PREVENTNATPHASE,
			FULLMEMBERDATE= I.FULLMEMBERDATE
		from	COUNTRYGROUP C
		join	CCImport_COUNTRYGROUP I	on ( I.TREATYCODE=C.TREATYCODE
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_COUNTRYGROUP  to public
go
