-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_DATESLOGIC
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_DATESLOGIC]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_DATESLOGIC.'
	drop procedure dbo.ip_cc_DATESLOGIC
	print '**** Creating procedure dbo.ip_cc_DATESLOGIC...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_DATESLOGIC
...
-- Prerequisite that the CCImport_DATESLOGIC table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_DATESLOGIC('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	DATETYPE= I.DATETYPE,
			...
			DISPLAYERRORFLAG= I.DISPLAYERRORFLAG,
			ERRORMESSAGE=replace( I.ERRORMESSAGE,char(10),char(13)+char(10)),
			INHERITED= I.INHERITED
		from	DATESLOGIC C
		join	CCImport_DATESLOGIC I	on ( I.CRITERIANO=C.CRITERIANO
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_DATESLOGIC  to public
go
