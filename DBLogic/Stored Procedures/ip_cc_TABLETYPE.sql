-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_TABLETYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_TABLETYPE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_TABLETYPE.'
	drop procedure dbo.ip_cc_TABLETYPE
	print '**** Creating procedure dbo.ip_cc_TABLETYPE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_TABLETYPE
...
-- Prerequisite that the CCImport_TABLETYPE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_TABLETYPE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update TABLETYPE
		set	TABLENAME= I.TABLENAME,
			MODIFIABLE= I.MODIFIABLE,
			ACTIVITYFLAG= I.ACTIVITYFLAG,
			DATABASETABLE= I.DATABASETABLE
		from	TABLETYPE C
		join	CCImport_TABLETYPE I	on ( I.TABLETYPE=C.TABLETYPE)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_TABLETYPE  to public
go
