-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_RECORDALTYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_RECORDALTYPE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_RECORDALTYPE.'
	drop procedure dbo.ip_cc_RECORDALTYPE
	print '**** Creating procedure dbo.ip_cc_RECORDALTYPE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_RECORDALTYPE
...
-- PROCEDURE :	ip_cc_RECORDALTYPE
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the RECORDALTYPE table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_RECORDALTYPE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_RECORDALTYPE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update RECORDALTYPE
		set	RECORDALTYPE= I.RECORDALTYPE,
			REQUESTEVENTNO= I.REQUESTEVENTNO,
			REQUESTACTION= I.REQUESTACTION,
			RECORDEVENTNO= I.RECORDEVENTNO,
			RECORDACTION= I.RECORDACTION
		from	RECORDALTYPE C
		join	CCImport_RECORDALTYPE I	on ( I.RECORDALTYPENO=C.RECORDALTYPENO)
" Set @sSQLString1="
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT RECORDALTYPE ON

		-- Insert the rows where existing key not found.
		Insert into RECORDALTYPE(
			RECORDALTYPENO,
			RECORDALTYPE,
			REQUESTEVENTNO,
			REQUESTACTION,
		...
		SET IDENTITY_INSERT RECORDALTYPE OFF
	End
End
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_RECORDALTYPE  to public
go
