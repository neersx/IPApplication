-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_TOPICCONTROL
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_TOPICCONTROL]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_TOPICCONTROL.'
	drop procedure dbo.ip_cc_TOPICCONTROL
	print '**** Creating procedure dbo.ip_cc_TOPICCONTROL...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_TOPICCONTROL
...
-- PROCEDURE :	ip_cc_TOPICCONTROL
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the TOPICCONTROL table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_TOPICCONTROL table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_TOPICCONTROL('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update TOPICCONTROL
		set	WINDOWCONTROLNO= I.WINDOWCONTROLNO,
			TOPICNAME= I.TOPICNAME,
			TOPICSUFFIX= I.TOPICSUFFIX,
			ROWPOSITION= I.ROWPOSITION,
			COLPOSITION= I.COLPOSITION,
			TABCONTROLNO= I.TABCONTROLNO,
			TOPICTITLE=replace( I.TOPICTITLE,char(10),char(13)+char(10)),
			TOPICSHORTTITLE=replace( I.TOPICSHORTTITLE,char(10),char(13)+char(10)),
			TOPICDESCRIPTION=replace( I.TOPICDESCRIPTION,char(10),char(13)+char(10)),
		...
		from	TOPICCONTROL C
		join	CCImport_TOPICCONTROL I	on ( I.TOPICCONTROLNO=C.TOPICCONTROLNO)
" Set @sSQLString1="
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT TOPICCONTROL ON
		...
		select
			...
			I.COLPOSITION,
			I.TABCONTROLNO,
			replace( I.TOPICTITLE,char(10),char(13)+char(10)),
			replace( I.TOPICSHORTTITLE,char(10),char(13)+char(10)),
			replace( I.TOPICDESCRIPTION,char(10),char(13)+char(10)),
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_TOPICCONTROL  to public
go
