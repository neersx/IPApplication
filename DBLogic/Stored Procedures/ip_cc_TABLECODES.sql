-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_TABLECODES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_TABLECODES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_TABLECODES.'
	drop procedure dbo.ip_cc_TABLECODES
	print '**** Creating procedure dbo.ip_cc_TABLECODES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_TABLECODES
...
-- Prerequisite that the CCImport_TABLECODES table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_TABLECODES('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update TABLECODES
		set	TABLETYPE= I.TABLETYPE,
			DESCRIPTION= I.DESCRIPTION,
			USERCODE= I.USERCODE,
			BOOLEANFLAG= I.BOOLEANFLAG
		from	TABLECODES C
		join	CCImport_TABLECODES I	on ( I.TABLECODE=C.TABLECODE)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_TABLECODES  to public
go
