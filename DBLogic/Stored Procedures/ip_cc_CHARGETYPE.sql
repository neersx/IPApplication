-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_CHARGETYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_CHARGETYPE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_CHARGETYPE.'
	drop procedure dbo.ip_cc_CHARGETYPE
	print '**** Creating procedure dbo.ip_cc_CHARGETYPE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_CHARGETYPE
...
-- PROCEDURE :	ip_cc_CHARGETYPE
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the CHARGETYPE table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_CHARGETYPE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_CHARGETYPE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update CHARGETYPE
		set	CHARGEDESC= I.CHARGEDESC,
			USEDASFLAG= I.USEDASFLAG,
			CHARGEDUEEVENT= I.CHARGEDUEEVENT,
			CHARGEINCURREDEVENT= I.CHARGEINCURREDEVENT,
			PUBLICFLAG= I.PUBLICFLAG
		from	CHARGETYPE C
		join	CCImport_CHARGETYPE I	on ( I.CHARGETYPENO=C.CHARGETYPENO)
" Set @sSQLString1="
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT CHARGETYPE ON
		...
		SET IDENTITY_INSERT CHARGETYPE OFF
	End
End
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_CHARGETYPE  to public
go
