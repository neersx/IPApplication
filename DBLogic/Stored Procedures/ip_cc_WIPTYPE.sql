-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_WIPTYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_WIPTYPE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_WIPTYPE.'
	drop procedure dbo.ip_cc_WIPTYPE
	print '**** Creating procedure dbo.ip_cc_WIPTYPE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_WIPTYPE
...
-- PROCEDURE :	ip_cc_WIPTYPE
-- VERSION :	1
-- DESCRIPTION:	The comparison/display and merging of imported data for the WIPTYPE table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_WIPTYPE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_WIPTYPE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update WIPTYPE
		set	CATEGORYCODE= I.CATEGORYCODE,
			DESCRIPTION= I.DESCRIPTION,
			CONSOLIDATE= I.CONSOLIDATE,
			WIPTYPESORT= I.WIPTYPESORT,
			RECORDASSOCDETAILS= I.RECORDASSOCDETAILS,
			EXCHSCHEDULEID= I.EXCHSCHEDULEID,
			WRITEDOWNPRIORITY= I.WRITEDOWNPRIORITY,
			WRITEUPALLOWED= I.WRITEUPALLOWED
		from	WIPTYPE C
		join	CCImport_WIPTYPE I	on ( I.WIPTYPEID=C.WIPTYPEID)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_WIPTYPE  to public
go
