-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_VALIDBASIS
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_VALIDBASIS]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_VALIDBASIS.'
	drop procedure dbo.ip_cc_VALIDBASIS
	print '**** Creating procedure dbo.ip_cc_VALIDBASIS...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_VALIDBASIS
...
-- Prerequisite that the CCImport_VALIDBASIS table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_VALIDBASIS('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		from	VALIDBASIS C
		join	CCImport_VALIDBASIS I	on ( I.COUNTRYCODE=C.COUNTRYCODE
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_VALIDBASIS  to public
go
