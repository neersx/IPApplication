-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_NUMBERTYPES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_NUMBERTYPES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_NUMBERTYPES.'
	drop procedure dbo.ip_cc_NUMBERTYPES
	print '**** Creating procedure dbo.ip_cc_NUMBERTYPES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_NUMBERTYPES
...
-- Prerequisite that the CCImport_NUMBERTYPES table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_NUMBERTYPES('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	DESCRIPTION= I.DESCRIPTION,
			RELATEDEVENTNO= I.RELATEDEVENTNO,
			ISSUEDBYIPOFFICE= I.ISSUEDBYIPOFFICE,
			DISPLAYPRIORITY= I.DISPLAYPRIORITY
		from	NUMBERTYPES C
		join	CCImport_NUMBERTYPES I	on ( I.NUMBERTYPE=C.NUMBERTYPE)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_NUMBERTYPES  to public
go
