-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_ATTRIBUTES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_ATTRIBUTES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_ATTRIBUTES.'
	drop procedure dbo.ip_cc_ATTRIBUTES
	print '**** Creating procedure dbo.ip_cc_ATTRIBUTES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_ATTRIBUTES
...
-- PROCEDURE :	ip_cc_ATTRIBUTES
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the ATTRIBUTES table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_ATTRIBUTES table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_ATTRIBUTES('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		from	ATTRIBUTES C
		join	CCImport_ATTRIBUTES I	on ( I.ATTRIBUTEID=C.ATTRIBUTEID)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_ATTRIBUTES  to public
go
