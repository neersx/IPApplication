-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_RulesCASECATEGORY
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_RulesCASECATEGORY]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_RulesCASECATEGORY.'
	drop procedure dbo.ip_RulesCASECATEGORY
	print '**** Creating procedure dbo.ip_RulesCASECATEGORY...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_RulesCASECATEGORY
...
-- PROCEDURE :	ip_RulesCASECATEGORY
-- VERSION :	4
-- DESCRIPTION:	The comparison/display and merging of imported data for the CASECATEGORY table
-- CALLED BY :	
...
-- Prerequisite that the IMPORTED_CASECATEGORY table has been loaded
...
Begin
	...
	Set @sSQLString="select @bOriginalKeyColumnExists = 1 
                         from syscolumns 
			 where (name = 'ORIGINAL_KEY') and id = object_id('"+@sUserName+".Imported_CASECATEGORY')"
	...
	Begin
		Set @sSQLString="ALTER TABLE "+@sUserName+".Imported_CASECATEGORY ADD ORIGINAL_KEY NVARCHAR(50)"
		exec @ErrorCode=sp_executesql @sSQLString

		-- Now save the original key value
		-- CASETYPE and CASECATEGORY are to be concatenated to uniquely
		-- identify the key.
		If @ErrorCode=0
		Begin
			Set @sSQLString=
			"UPDATE "+@sUserName+".Imported_CASECATEGORY
			SET ORIGINAL_KEY=RTRIM(CASETYPE+CASECATEGORY)"
	...
	Begin
		Set @sSQLString=
			"UPDATE "+@sUserName+".Imported_CASECATEGORY
			-- strip off the CASETYPE when setting the CaseCategory
			SET CASECATEGORY = substring(isnull(M.MAPVALUE, C.ORIGINAL_KEY),2,2)
			FROM "+@sUserName+".Imported_CASECATEGORY C
...
Begin
	...
	Begin
		...
		select	3			as 'Comparison',
			...
			I.CASECATEGORY		as 'Imported Category Code',
			C.CASECATEGORYDESC	as 'Imported Category',
			CT.CASETYPEDESC		as 'Case Type',
			C.CASECATEGORY		as 'Category Code',
			C.CASECATEGORYDESC	as 'Category'
		from "+@sUserName+".Imported_CASECATEGORY I"
		Set @sSQLString2="
...
Begin
	Set @sSQLString="
	select CASETYPE+CASECATEGORY,'{'+CASETYPE+CASECATEGORY+'}'+CASECATEGORYDESC
	from CASECATEGORY
...
Begin
	...
	select	I.ORIGINAL_KEY,
		I.CASECATEGORYDESC,
		CASE WHEN (I.CASETYPE+I.CASECATEGORY = I.ORIGINAL_KEY)THEN NULL 
			ELSE I.CASETYPE+I.CASECATEGORY END
	from "+@sUserName+".Imported_CASECATEGORY I
	left join CASECATEGORY C on (C.CASETYPE    =I.CASETYPE
				 and C.CASECATEGORY=I.CASECATEGORY)
	order by 1"
...
Begin
	...
	Begin
		Set @sSQLString="
			...
			AND MAPTABLE = 'CASECATEGORY'
			AND MAPCOLUMN = 'CASECATEGORY'
			AND SOURCEVALUE IN (
				SELECT SOURCEVALUE
	...
	Begin
		...
		select	
			...
			left join DATAMAP DM on (DM.SOURCENO = @pnSourceNo
					     ...
					     and DM.MAPTABLE = 'CASECATEGORY'
					     and DM.MAPCOLUMN = 'CASECATEGORY')
...
RETURN @ErrorCode
go
grant execute on dbo.ip_RulesCASECATEGORY  to public
go
