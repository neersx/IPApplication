-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_APPLICATIONBASIS
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_APPLICATIONBASIS]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_APPLICATIONBASIS.'
	drop procedure dbo.ip_cc_APPLICATIONBASIS
	print '**** Creating procedure dbo.ip_cc_APPLICATIONBASIS...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_APPLICATIONBASIS
...
-- PROCEDURE :	ip_cc_APPLICATIONBASIS
-- VERSION :	1
...
-- Prerequisite that the CCImport_APPLICATIONBASIS table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_APPLICATIONBASIS('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	BASISDESCRIPTION= I.BASISDESCRIPTION,
			CONVENTION= I.CONVENTION
		from	APPLICATIONBASIS C
		join	CCImport_APPLICATIONBASIS I	on ( I.BASIS=C.BASIS)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_APPLICATIONBASIS  to public
go
