-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_PROPERTYTYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_PROPERTYTYPE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_PROPERTYTYPE.'
	drop procedure dbo.ip_cc_PROPERTYTYPE
	print '**** Creating procedure dbo.ip_cc_PROPERTYTYPE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_PROPERTYTYPE
...
-- Prerequisite that the CCImport_PROPERTYTYPE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_PROPERTYTYPE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	PROPERTYNAME= I.PROPERTYNAME,
			ALLOWSUBCLASS= I.ALLOWSUBCLASS,
			CRMONLY= I.CRMONLY
		from	PROPERTYTYPE C
		join	CCImport_PROPERTYTYPE I	on ( I.PROPERTYTYPE=C.PROPERTYTYPE)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_PROPERTYTYPE  to public
go
