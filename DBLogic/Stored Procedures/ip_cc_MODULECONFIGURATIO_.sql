-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_MODULECONFIGURATIO_
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_MODULECONFIGURATIO_]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_MODULECONFIGURATIO_.'
	drop procedure dbo.ip_cc_MODULECONFIGURATIO_
	print '**** Creating procedure dbo.ip_cc_MODULECONFIGURATIO_...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_MODULECONFIGURATIO_
...
-- PROCEDURE :	ip_cc_MODULECONFIGURATIO_
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the MODULECONFIGURATION table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_MODULECONFIGURATION table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_MODULECONFIGURATIO_('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	IDENTITYID= I.IDENTITYID,
			TABID= I.TABID,
			MODULEID= I.MODULEID,
			MODULESEQUENCE= I.MODULESEQUENCE,
			PANELLOCATION= I.PANELLOCATION,
			PORTALID= I.PORTALID
		from	MODULECONFIGURATION C
		join	CCImport_MODULECONFIGURATION I	on ( I.CONFIGURATIONID=C.CONFIGURATIONID)
" Set @sSQLString1="
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT MODULECONFIGURATION ON
		...
		SET IDENTITY_INSERT MODULECONFIGURATION OFF
	End
End
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_MODULECONFIGURATIO_  to public
go
