-----------------------------------------------------------------------------------------------------------------------------
-- Creation of gl_RWCalculateBudgetData
-----------------------------------------------------------------------------------------------------------------------------
If exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[gl_RWCalculateBudgetData]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
Begin
	Print '**** Drop Stored Procedure dbo.gl_RWCalculateBudgetData.'
	Drop procedure [dbo].[gl_RWCalculateBudgetData]
End
Print '**** Creating Stored Procedure dbo.gl_RWCalculateBudgetData...'
Print ''
GO
...
CREATE PROCEDURE dbo.gl_RWCalculateBudgetData
(
	...
	@psColumnName 		nvarchar(128),	-- A column name in @psTempTableName, which will stores the sum of budget movements.
	@pnPeriodFrom 		int,		-- Indicates from which period the budget data will be returned.
	@pnPeriodTo 		int		-- Indicates to which period the budget data will be returned.
)
as
-- PROCEDURE:	gl_RWCalculateBudgetData
-- VERSION:	4
-- SCOPE:	InPro
-- DESCRIPTION:	Calculates budget related data
...
-- 22 May 2006	AT	12563	3	Added isnulls to cater for NULL BUDGETAMOUNTs
...
Begin
	Set @sSql = 'Update ' + @psTempTableName + ' 
			...
			SUM (isnull(BUDGETAMOUNT, 0))
			from  	(
		select  A.ENTITYNO, A.LEDGERACCOUNTID,A.PROFITCENTRECODE, A.PERIODID, A.BUDGETAMOUNT
		...
		and 	A.BUDGETAMOUNT IS NOT NULL
		UNION ALL
		select A.ENTITYNO, A.LEDGERACCOUNTID,A.PROFITCENTRECODE, A.PERIODID, A.BUDGETAMOUNT - B.BUDGETAMOUNT
...
Grant execute on dbo.gl_RWCalculateBudgetData to public
GO
