-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_DOCUMENTDEFINITION
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_DOCUMENTDEFINITION]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_DOCUMENTDEFINITION.'
	drop procedure dbo.ip_cc_DOCUMENTDEFINITION
	print '**** Creating procedure dbo.ip_cc_DOCUMENTDEFINITION...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_DOCUMENTDEFINITION
...
-- PROCEDURE :	ip_cc_DOCUMENTDEFINITION
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the DOCUMENTDEFINITION table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_DOCUMENTDEFINITION table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_DOCUMENTDEFINITION('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	LETTERNO= I.LETTERNO,
			NAME= I.NAME,
			DESCRIPTION=replace( I.DESCRIPTION,char(10),char(13)+char(10)),
			CANFILTERCASES= I.CANFILTERCASES,
			CANFILTEREVENTS= I.CANFILTEREVENTS,
			SENDERREQUESTTYPE= I.SENDERREQUESTTYPE
		from	DOCUMENTDEFINITION C
		join	CCImport_DOCUMENTDEFINITION I	on ( I.DOCUMENTDEFID=C.DOCUMENTDEFID)
" Set @sSQLString1="
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT DOCUMENTDEFINITION ON
		...
		SET IDENTITY_INSERT DOCUMENTDEFINITION OFF
	End
End
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_DOCUMENTDEFINITION  to public
go
