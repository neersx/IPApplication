-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_RulesTABLECODES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_RulesTABLECODES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_RulesTABLECODES.'
	drop procedure dbo.ip_RulesTABLECODES
	print '**** Creating procedure dbo.ip_RulesTABLECODES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_RulesTABLECODES
...
-- PROCEDURE :	ip_RulesTABLECODES
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the TABLECODES table
-- CALLED BY :	
...
-- Prerequisite that the IMPORTED_TABLECODES table has been loaded
...
Begin
	...
	Set @sSQLString="select @bOriginalKeyColumnExists = 1 
                         from syscolumns 
			 where (name = 'ORIGINAL_KEY') and id = object_id('"+@sUserName+".Imported_TABLECODES')"
	...
	Begin
		Set @sSQLString="ALTER TABLE "+@sUserName+".Imported_TABLECODES ADD ORIGINAL_KEY NVARCHAR(50)"
		exec @ErrorCode=sp_executesql @sSQLString
		...
		Begin
			Set @sSQLString=
			"UPDATE "+@sUserName+".Imported_TABLECODES
			SET ORIGINAL_KEY=RTRIM(TABLECODE)"
	...
	Begin
		Set @sSQLString=
			"UPDATE "+@sUserName+".Imported_TABLECODES
			SET TABLECODE = isnull(M.MAPVALUE, C.ORIGINAL_KEY)
			FROM "+@sUserName+".Imported_TABLECODES C
...
Begin
	...
	Begin
		...
		select	3		as 'Comparison',
			NULL		as Match,
			T.TABLENAME	as 'Imported Table Type',
			I.TABLECODE	as 'Imported Table Code',
			I.DESCRIPTION	as 'Imported Description',
			T.TABLENAME	as 'Table Type',
			C.TABLECODE	as 'Table Code',
			C.DESCRIPTION	as 'Description'
		from "+@sUserName+".Imported_TABLECODES I
		join "+@sUserName+".Imported_TABLETYPE T	on (T.TABLETYPE=I.TABLETYPE)"
		Set @sSQLString2="
		...
		from "+@sUserName+".Imported_TABLECODES I
		join "+@sUserName+".Imported_TABLETYPE T	on (T.TABLETYPE=I.TABLETYPE)"
		Set @sSQLString3="	left join TABLECODES C on( C.TABLECODE=I.TABLECODE)
...
Begin
	Set @sSQLString="
	select TABLECODE,'{'+convert(varchar,TABLECODE)+'}'+DESCRIPTION
	from TABLECODES
	order by TABLECODE"
...
Begin
	...
	select	I.ORIGINAL_KEY,
		I.DESCRIPTION,
		CASE WHEN (I.TABLECODE = I.ORIGINAL_KEY)THEN NULL 
			ELSE I.TABLECODE END
	from "+@sUserName+".Imported_TABLECODES I
	left join TABLECODES C on C.TABLECODE = I.TABLECODE
	order by 1"
...
Begin
	...
	Begin
		Set @sSQLString="
			...
			AND MAPTABLE = 'TABLECODES'
			AND MAPCOLUMN = 'TABLECODE'
			AND SOURCEVALUE IN (
				SELECT SOURCEVALUE
	...
	Begin
		...
		select	
			...
			left join DATAMAP DM on (DM.SOURCENO = @pnSourceNo
					     ...
					     and DM.MAPTABLE = 'TABLECODES'
					     and DM.MAPCOLUMN = 'TABLECODE')
...
RETURN @ErrorCode
go
grant execute on dbo.ip_RulesTABLECODES  to public
go
