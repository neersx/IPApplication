-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_SELECTIONTYPES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_SELECTIONTYPES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_SELECTIONTYPES.'
	drop procedure dbo.ip_cc_SELECTIONTYPES
	print '**** Creating procedure dbo.ip_cc_SELECTIONTYPES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_SELECTIONTYPES
...
-- PROCEDURE :	ip_cc_SELECTIONTYPES
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the SELECTIONTYPES table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_SELECTIONTYPES table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_SELECTIONTYPES('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	MINIMUMALLOWED= I.MINIMUMALLOWED,
			MAXIMUMALLOWED= I.MAXIMUMALLOWED,
			MODIFYBYSERVICE= I.MODIFYBYSERVICE
		from	SELECTIONTYPES C
		join	CCImport_SELECTIONTYPES I	on ( I.PARENTTABLE=C.PARENTTABLE
						and I.TABLETYPE=C.TABLETYPE)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_SELECTIONTYPES  to public
go
