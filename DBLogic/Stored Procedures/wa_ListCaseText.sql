-----------------------------------------------------------------------------------------------------------------------------
-- Creation of dbo.wa_ListCaseText
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[wa_ListCaseText]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.wa_ListCaseText'
	drop procedure [dbo].[wa_ListCaseText]
	print '**** Creating procedure dbo.wa_ListCaseText...'
	print ''
end
go

CREATE PROCEDURE [dbo].[wa_ListCaseText]
	@pnCaseId 	int
AS
-- PROCEDURE :	wa_ListCaseText
-- VERSION :	2.2.0
-- DESCRIPTION:	Returns a list of Text (other than Goods & Services) for a given Case passed as a parameter.
-- CALLED BY :	
...
begin
	...
	Begin
		...
		select	DESCRIPTION = TT.TEXTDESCRIPTION, 
				...
				END
		from	CASETEXT CT
		join	TEXTTYPE TT	on (CT.TEXTTYPE	= TT.TEXTTYPE)
		where	CT.TEXTTYPE not in ( 'T','G')
		...
		ORDER BY  TT.TEXTDESCRIPTION, CT.LANGUAGE
...
grant execute on [dbo].[wa_ListCaseText] to public
go
