-----------------------------------------------------------------------------------------------------------------------------
-- Creation of pi_ListInstructionDefinition
-----------------------------------------------------------------------------------------------------------------------------
If exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[pi_ListInstructionDefinition]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
Begin
	Print '**** Drop Stored Procedure dbo.pi_ListInstructionDefinition.'
	Drop procedure [dbo].[pi_ListInstructionDefinition]
End
Print '**** Creating Stored Procedure dbo.pi_ListInstructionDefinition...'
Print ''
GO
...
CREATE PROCEDURE dbo.pi_ListInstructionDefinition
(
	@pnRowCount			int 		= null	output,
	@pnUserIdentityId		int,		-- Mandatory
	@psCulture			nvarchar(10)	= null, -- the language in which output is to be expressed
	@pnQueryContextKey		int		= 350, 	-- The key for the context of the query (default output requests).
...
as
-- PROCEDURE:	pi_ListInstructionDefinition
-- VERSION:	3
-- DESCRIPTION:	Returns the requested information for instruction definitions that match the filter criteria provided.

-- MODIFICATIONS :
-- Date		Who	Change	Version	Description
-- -----------	-------	-------	-------	----------------------------------------------- 
-- 29 Nov 2006	AU	RFC4634	1	Procedure created
-- 20 Dec 2006	AU	RFC4634	2	Correctly default the @pnQueryContextKey
...
Declare @nDefinitionKey 			nvarchar(3)	-- The primary key of the currency. This is user modifiable, so also acts as the currency code.The data is held on the database in upper case. Always convert search string to upper case.
Declare @nDefinitionKeyOperator			tinyint		
Declare @sInstructionName			nvarchar(40)	-- The description of the currency. Case insensitive search.
Declare @nInstructionNameOperator		tinyint
...
set 	@sFrom					= char(10)+"From INSTRUCTIONDEFINITION IDFN"
...
Else
Begin
	-- Default @pnQueryContextKey to 350.
	Set @pnQueryContextKey = isnull(@pnQueryContextKey, 350)
...
Begin
	...
	If @nErrorCode=0
	Begin
		If @sColumn='DefinitionKey'
		Begin
			Set @sTableColumn='IDFN.DEFINITIONID'
		End
		Else 
		If @sColumn='InstructionName'
		Begin
			Set @sTableColumn=dbo.fn_SqlTranslatedColumn('INSTRUCTIONDEFINITION','INSTRUCTIONNAME',null,'IDFN',@sLookupCulture,@pbCalledFromCentura)
		End
		Else 
		If @sColumn = 'InstructionExplanation'
		Begin
			Set @sTableColumn=dbo.fn_SqlTranslatedColumn('INSTRUCTIONDEFINITION','EXPLANATION',null,'IDFN',@sLookupCulture,@pbCalledFromCentura) 
		End
...
Begin
	...
	Set @sSQLString = 	
	"Select @nDefinitionKey			= upper(DefinitionKey),"+CHAR(10)+
	"	@nDefinitionKeyOperator		= DefinitionKeyOperator,"+CHAR(10)+
	"	@sInstructionName		= upper(InstructionName),"+CHAR(10)+
	"	@nInstructionNameOperator	= upper(InstructionNameOperator),"+CHAR(10)+
	"	@sPickListSearch		= upper(PickListSearch)"+CHAR(10)+	
	"from	OPENXML (@idoc, '/pi_ListInstructionDefinition/FilterCriteria',2)"+CHAR(10)+
	"	WITH ("+CHAR(10)+
	"	      DefinitionKey		int		'DefinitionKey/text()',"+CHAR(10)+
	"	      DefinitionKeyOperator	tinyint		'DefinitionKey/@Operator/text()',"+CHAR(10)+
	"	      InstructionName		nvarchar(50)	'InstructionName/text()',"+CHAR(10)+	
	"	      InstructionNameOperator	tinyint		'InstructionName/@Operator/text()',"+CHAR(10)+
	...
	exec @nErrorCode = sp_executesql @sSQLString,
				N'@idoc				int,
				  ...
				  @sInstructionName		nvarchar(50)		output,
				  @nInstructionNameOperator	tinyint			output,	
				  ...
				  @sInstructionName		= @sInstructionName		output,
				  @nInstructionNameOperator	= @nInstructionNameOperator	output,
	...
	Begin	
		...
		Begin
			Set @sWhere = @sWhere+char(10)+"and IDFN.DEFINITIONID " + dbo.fn_ConstructOperator(@nDefinitionKeyOperator,@String,@nDefinitionKey, null,0)
		End		

		If @sInstructionName is not NULL 
		or @nInstructionNameOperator between 2 and 6
		Begin
			Set @sWhere = @sWhere+char(10)+"and upper("+dbo.fn_SqlTranslatedColumn('INSTRUCTIONDEFINITION','INSTRUCTIONNAME',null,'IDFN',@sLookupCulture,@pbCalledFromCentura)+") " + dbo.fn_ConstructOperator(@nInstructionNameOperator,@String,@sInstructionName, null,0)
		End	
		
		If @sPickListSearch is not null
		Begin
			Set @sWhere=@sWhere+char(10)+"and upper(IDFN.[INSTRUCTIONNAME]) Like '"+@sPickListSearch+"%'"
		End
...
Grant execute on dbo.pi_ListInstructionDefinition to public
GO
