-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_COUNTRYTEXT
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_COUNTRYTEXT]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_COUNTRYTEXT.'
	drop procedure dbo.ip_cc_COUNTRYTEXT
	print '**** Creating procedure dbo.ip_cc_COUNTRYTEXT...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_COUNTRYTEXT
...
-- Prerequisite that the CCImport_COUNTRYTEXT table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_COUNTRYTEXT('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update COUNTRYTEXT
		set	PROPERTYTYPE= I.PROPERTYTYPE,
			MODIFIEDDATE= I.MODIFIEDDATE,
			LANGUAGE= I.LANGUAGE,
			USEFLAG= I.USEFLAG,
			COUNTRYTEXT= replace(CAST(I.COUNTRYTEXT as NVARCHAR(MAX)),char(10),char(13)+char(10))
		from	COUNTRYTEXT C
		join	CCImport_COUNTRYTEXT I	on ( I.COUNTRYCODE=C.COUNTRYCODE
						and I.TEXTID=C.TEXTID
...
"
	...
	/**************** Data Insert ***************************************/
	 ...
	 I.LANGUAGE,
	 I.USEFLAG,
	 replace(CAST(I.COUNTRYTEXT as NVARCHAR(MAX)),char(10),char(13)+char(10))
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_COUNTRYTEXT  to public
go
