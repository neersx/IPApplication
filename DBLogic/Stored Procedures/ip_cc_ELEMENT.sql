-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_ELEMENT
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_ELEMENT]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_ELEMENT.'
	drop procedure dbo.ip_cc_ELEMENT
	print '**** Creating procedure dbo.ip_cc_ELEMENT...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_ELEMENT
...
-- PROCEDURE :	ip_cc_ELEMENT
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the ELEMENT table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_ELEMENT table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_ELEMENT('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		from	ELEMENT C
		join	CCImport_ELEMENT I	on ( I.ELEMENTNO=C.ELEMENTNO)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_ELEMENT  to public
go
