-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_RulesNUMBERTYPES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_RulesNUMBERTYPES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_RulesNUMBERTYPES.'
	drop procedure dbo.ip_RulesNUMBERTYPES
	print '**** Creating procedure dbo.ip_RulesNUMBERTYPES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_RulesNUMBERTYPES
...
-- PROCEDURE :	ip_RulesNUMBERTYPES
-- VERSION :	3
-- DESCRIPTION:	The comparison/display and merging of imported data for the NUMBERTYPES table
-- CALLED BY :	
...
-- Prerequisite that the IMPORTED_NUMBERTYPES table has been loaded
...
Begin
	...
	Set @sSQLString="select @bOriginalKeyColumnExists = 1 
                         from syscolumns 
			 where (name = 'ORIGINAL_KEY') and id = object_id('"+@sUserName+".Imported_NUMBERTYPES')"
	...
	Begin
		Set @sSQLString="ALTER TABLE "+@sUserName+".Imported_NUMBERTYPES ADD ORIGINAL_KEY NVARCHAR(50)"
		exec @ErrorCode=sp_executesql @sSQLString
		...
		Begin
			Set @sSQLString=
			"UPDATE "+@sUserName+".Imported_NUMBERTYPES
			SET ORIGINAL_KEY=RTRIM(NUMBERTYPE)"
	...
	Begin
		Set @sSQLString=
			"UPDATE "+@sUserName+".Imported_NUMBERTYPES
			SET NUMBERTYPE = isnull(M.MAPVALUE, C.ORIGINAL_KEY)
			FROM "+@sUserName+".Imported_NUMBERTYPES C
...
Begin
	...
	Begin
		...
		select	3			as 'Comparison',
			NULL			as Match,
			I.NUMBERTYPE		as 'Imported Number Type',
			C.DESCRIPTION		as 'Imported Description',
			E.EVENTDESCRIPTION	as 'Imported Related Event',
			dbo.fn_DisplayBoolean(I.ISSUEDBYIPOFFICE,"+CASE WHEN(@psCulture is null) THEN 'DEFAULT' ELSE "'"+ @psCulture+"'" END +",0)
						as 'Imported Issued By IP Office',
			C.DISPLAYPRIORITY	as 'Imported Display Priority',
			C.NUMBERTYPE		as 'Number Type',
			C.DESCRIPTION		as 'Description',
			E.EVENTDESCRIPTION	as 'Related Event',
			dbo.fn_DisplayBoolean(C.ISSUEDBYIPOFFICE,"+CASE WHEN(@psCulture is null) THEN 'DEFAULT' ELSE "'"+ @psCulture+"'" END +",0)
						as 'Issued By IP Office',
			C.DISPLAYPRIORITY	as 'Display Priority'
		from "+@sUserName+".Imported_NUMBERTYPES I
		left join "+@sUserName+".Imported_EVENTS E	on (E.EVENTNO=I.RELATEDEVENTNO)"
		Set @sSQLString2="
		...
		from "+@sUserName+".Imported_NUMBERTYPES I
		left join "+@sUserName+".Imported_EVENTS E	on (E.EVENTNO=I.RELATEDEVENTNO)"
		Set @sSQLString3="	left join NUMBERTYPES C on( C.NUMBERTYPE=I.NUMBERTYPE)
...
Begin
	Set @sSQLString="
	select NUMBERTYPE,'{'+NUMBERTYPE+'}'+DESCRIPTION
	from NUMBERTYPES
	order by NUMBERTYPE"
...
Begin
	...
	select	I.ORIGINAL_KEY,
		I.DESCRIPTION,
		CASE WHEN (I.NUMBERTYPE = I.ORIGINAL_KEY)THEN NULL 
			ELSE I.NUMBERTYPE END
	from "+@sUserName+".Imported_NUMBERTYPES I
	left join NUMBERTYPES C on C.NUMBERTYPE = I.NUMBERTYPE
	order by 1"
...
Begin
	...
	Begin
		Set @sSQLString="
			...
			AND MAPTABLE = 'NUMBERTYPES'
			AND MAPCOLUMN = 'NUMBERTYPE'
			AND SOURCEVALUE IN (
				SELECT SOURCEVALUE
	...
	Begin
		...
		select	
			...
			left join DATAMAP DM on (DM.SOURCENO = @pnSourceNo
					     ...
					     and DM.MAPTABLE = 'NUMBERTYPES'
					     and DM.MAPCOLUMN = 'NUMBERTYPE')
...
RETURN @ErrorCode
go
grant execute on dbo.ip_RulesNUMBERTYPES  to public
go
