-----------------------------------------------------------------------------------------------------------------------------
-- Creation of pr_ListSourceDocuments
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[pr_ListSourceDocuments]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.pr_ListSourceDocuments.'
	drop procedure dbo.pr_ListSourceDocuments
	print '**** Creating procedure dbo.pr_ListSourceDocuments...'
	print ''
end
go
...
CREATE PROCEDURE dbo.pr_ListSourceDocuments
...
-- PROCEDURE :	pr_ListSourceDocuments
-- VERSION :	4
...
-- ------------	-------	------		-------	----------------------------------------------- 
-- 05/03/2011	KR	RFC6563		1	Procedure created
-- 09/05/2011	JC	RFC10574	2	Add IsRecentEntriesOnly
-- 13/07/2011	DL	SQA19795	3	Specify collate database default for temp table.
-- 24/10/2016	AK	R72645	    4	Make compatible with case sensitive server with case insensitive database.
...
Begin
	...
	"from	OPENXML (@idoc, '/pr_ListSourceDocuments/FilterCriteria',2)"+CHAR(10)+
	"	WITH ("+CHAR(10)+
...
Begin
	...
	Begin
		...
		Begin
			Set @sTableColumn="ltrim(S.ISSUINGCOUNTRY + ' ' + coalesce(" +
			dbo.fn_SqlTranslatedColumn('SEARCHRESULTS','DESCRIPTION',null,'S',@sLookupCulture,@pbCalledFromCentura) +",S.PUBLICATION))"
		End
		...
		End
		Else If @sColumn='PublicationNumber'
		Begin
			Set @sTableColumn='S.PUBLICATION'
		End
		Else If @sColumn='IssuingCountryCode'
		Begin
			Set @sTableColumn='S.ISSUINGCOUNTRY'
		End
		Else If @sColumn='IssuingCountry'
		Begin
			Set @sTableColumn=dbo.fn_SqlTranslatedColumn('COUNTRY','COUNTRY',null,'C',@sLookupCulture,@pbCalledFromCentura)
		End
		Else If @sColumn='ReportIssuedDate'
		Begin
			Set @sTableColumn='S.ISSUEDDATE'
		End
		Else If @sColumn='ReportReceivedDate'
		Begin
			Set @sTableColumn='S.RECEIVEDDATE'
		End
...
begin
	...
	set @sWhere = char(10)+"WHERE ISSOURCEDOCUMENT  = 1"
	...
	begin
		Set @sWhere = @sWhere+char(10)+"and (ISSUINGCOUNTRY"+dbo.fn_ConstructOperator(@nPickListDescriptionOperator,@String,@sPickListDescription, null,0)+
		"or (PUBLICATION"+dbo.fn_ConstructOperator(@nPickListDescriptionOperator,@String,@sPickListDescription, null,0)+")"
	end	
	...
	Begin
		...
		Begin
			...
			Else
				...
				" or S.PUBLICATION Like "+dbo.fn_WrapQuotes(@sPickListSearch + '%', 0, 0) +
				" or S.ISSUINGCOUNTRY Like "+dbo.fn_WrapQuotes(@sPickListSearch + '%', 0, 0) +
				+")"
		End
		Else Begin
			set @sWhere=@sWhere+char(10)+"and ("+dbo.fn_SqlTranslatedColumn('SEARCHRESULTS','DESCRIPTION',null,'S',@sLookupCulture,@pbCalledFromCentura)+") Like "+dbo.fn_WrapQuotes(@sPickListSearch + '%', 0, 0) +
									"  or " + dbo.fn_SqlTranslatedColumn('SEARCHRESULTS','PUBLICATION',null,'S',@sLookupCulture,@pbCalledFromCentura)+") Like "+dbo.fn_WrapQuotes(@sPickListSearch + '%', 0, 0) +
									"  or " + dbo.fn_SqlTranslatedColumn('SEARCHRESULTS','ISSUINGCOUNTRY',null,'S',@sLookupCulture,@pbCalledFromCentura)+") Like "+dbo.fn_WrapQuotes(@sPickListSearch + '%', 0, 0) +
									+")"
		End
	End
...
grant execute on dbo.pr_ListSourceDocuments  to public
go
