-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ipw_ListItems
-----------------------------------------------------------------------------------------------------------------------------
If exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ipw_ListItems]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
Begin
	Print '**** Drop Stored Procedure dbo.ipw_ListItems.'
	Drop procedure [dbo].[ipw_ListItems]
	Print '**** Creating Stored Procedure dbo.ipw_ListItems...'
	Print ''
End
GO
...
CREATE PROCEDURE [dbo].[ipw_ListItems]
(
	@pnRowCount			int 		= null	output,
	@pnUserIdentityId		int,		-- Mandatory
	@psCulture			nvarchar(10)	= null, -- the language in which output is to be expressed
	@pnQueryContextKey		int		= 850, 	-- The key for the context of the query (default output requests).
...
as
-- PROCEDURE:	ipw_ListItems
-- VERSION:	4
-- DESCRIPTION:	Returns the requested information for Items that match the filter criteria provided.
-- COPYRIGHT:	Copyright CPA Software Solutions (Australia) Pty Limited
-- MODIFICATIONS :
-- Date		Who	Change	Version	Description
-- ------------	-------	------	-------	----------------------------------------------- 
-- 23 Sep 2010	DV	RFC9387	 1	Procedure created 
-- 12 Jul 2011	DL	SQA19795 2	Specify collate database default for temp table.
-- 20 Jan 2011  DV      RFC11605 3      Allow search by ITEM_ID
-- 07 Jun 2016  MS      RFC61513 4      Add isnull check for @nGroupCode before using it
...
Declare @bExists				bit
Declare @sGroupName				nvarchar(50)
...
set 	@sFrom					= char(10)+"From ITEM I"
set 	@sWhere 				= char(10)+"	WHERE 1=1"
set		@sOrder					= char(10)+" I.ITEM_NAME"
...
Else
Begin
	-- Default @pnQueryContextKey to 850.
	Set @pnQueryContextKey = isnull(@pnQueryContextKey, 850)
...
Begin
	...
	Begin
		If @sColumn='ItemKey'
		Begin
			Set @sTableColumn='I.ITEM_ID'
		End
		If @sColumn='ItemName'
		Begin
			Set @sTableColumn='I.ITEM_NAME'
		End
		Else 
		If @sColumn = 'ItemDescription'
		Begin
			Set @sTableColumn=dbo.fn_SqlTranslatedColumn('ITEM','ITEM_DESCRIPTION',null,'I',@sLookupCulture,@pbCalledFromCentura) 
		End
...
Begin
	...
	"@nItemKey		= ItemKey,"+CHAR(10)+
	"@sGroupName		= upper(GroupName)"+CHAR(10)+	
	"from	OPENXML (@idoc, '/ipw_ListItems/FilterCriteria',2)"+CHAR(10)+
	"	WITH ("+CHAR(10)+	
 	"	      PickListSearch		nvarchar(50)	'PickListSearch/text()',"+CHAR(10)+
 	"	      ItemKey			int		'ItemKey/text()',"+CHAR(10)+	
	"	      GroupName				nvarchar(50)	'GroupName/text()'"+CHAR(10)+	
     	"     		)"
	...
	exec @nErrorCode = sp_executesql @sSQLString,
				N'@idoc				int,
				  ...
				  @nItemKey			int			output,
				  @sGroupName			nvarchar(50)		output',
				  @idoc				= @idoc,
				  @sPickListSearch		= @sPickListSearch	output,	
				  @nItemKey			= @nItemKey		output,		
				  @sGroupName			= @sGroupName	output	
	...
	If @nErrorCode = 0
	Begin
	        If @nItemKey is not null
		Begin
			Set @sWhere=@sWhere+char(10)+"and (I.ITEM_ID=" + CONVERT(nvarchar,@nItemKey) +")"
		End
		...
		Begin
			Set @bExists = 0
				...
				Set @sSQLString = "Select @bExists=1"+char(10)+
						  "from ITEM I"+char(10)+
						  @sWhere+
						  "and (I.ITEM_NAME=" + dbo.fn_WrapQuotes(@sPickListSearch,0,0)+")"
				...
				If @bExists=1
				Begin
					Set @sWhere=@sWhere+char(10)+"and (I.ITEM_NAME=" + dbo.fn_WrapQuotes(@sPickListSearch,0,0)+")"
				End
				Else
				Begin
					Set @sWhere=@sWhere+char(10)+"and (I.ITEM_NAME like " + dbo.fn_WrapQuotes(@sPickListSearch + '%',0,0)+ 
								     " or upper("+dbo.fn_SqlTranslatedColumn('ITEM','ITEM_DESCRIPTION',null,'I',@sLookupCulture,@pbCalledFromCentura)+") like " + dbo.fn_WrapQuotes(@sPickListSearch + '%',0,0)+")"
				End
		End
	End
	...
	If @nErrorCode = 0 and ISNULL(@nGroupCode,0) <> 0
	Begin
		Set @sFrom = @sFrom +CHAR(10)+'left join ITEM_GROUP IG on (IG.ITEM_ID=I.ITEM_ID)'
		Set @sWhere=@sWhere+char(10)+"and (IG.GROUP_CODE=" + Convert(nvarchar(10),@nGroupCode) +")"
	End
End
...
Grant execute on dbo.ipw_ListItems to public
GO
