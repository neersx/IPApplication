-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ipw_ListCaseControlCriteria
-----------------------------------------------------------------------------------------------------------------------------
If exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[ipw_ListCaseControlCriteria]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
Begin
	Print '**** Drop Stored Procedure dbo.ipw_ListCaseControlCriteria.'
	Drop procedure [dbo].[ipw_ListCaseControlCriteria]
End
Print '**** Creating Stored Procedure dbo.ipw_ListCaseControlCriteria...'
Print ''
GO
...
CREATE PROCEDURE dbo.ipw_ListCaseControlCriteria
(
...
as
-- PROCEDURE:	ipw_ListCaseControlCriteria
-- VERSION:	14
...
-- -----------	-------	------	-------	----------------------------------------------- 
-- 20 Nov 2008	NG	R7208	1	Procedure created
-- 18 Mar 2009	LP	R7208	2	Return ParentCriteria, HasChildren and CriteriaName columns.
-- 25 Mar 2009  LP	R7208	3	Fix @sSQLString length issue with pt-BR culture
-- 15 Apr 2009	NG	R7845	4	Fix to cater the situation where the "Valid" table does not have an entry for an explicit CountryCode.
-- 22 Jul 2009	KR	R8263	5	Fix the syntax error shown when the firm has no crm license 
-- 09 Sep 2009	LP	R8047	6	Add new ProfileKey filter criteria.
-- 12 Nov 2009	KR	R8637	7	Set the order by to DESC for BESTFIT
-- 05 Jan 2010  KR	R8171	8	Do not restrict rows to the default case program and allow rows from other logical programs to be listed as well.
-- 08 Feb 2010	MS	R7329	9	Search based on CriteriaNo
-- 16 Mar 2010  DV	R8935	10	Added a new column IsNewCaseCriteria
-- 15 Apr 2010	KR	R9110	11	Fix the where clause so that the Property Type is checked for null in the CRM user check
-- 07 Jul 2011	DL	R10830	12	Specify database collation default to temp table columns of type varchar, nvarchar and char
-- 14 Oct 2011	KR	R10427	13	Increase the length of variables @sSQLString and @sSelect
-- 30 May 2014	MF	R34931	14	If Exact Match search then Unknown flags set to zero should be ignored by changing to NULL.
...
declare	@pnPropertyUnknown	decimal(1,0)
declare	@pnCountryUnknown	decimal(1,0)
declare	@pnCategoryUnknown	decimal(1,0)
declare	@pnSubTypeUnknown	decimal(1,0)
...
Begin
	...
	Set @pnPropertyUnknown	= null
	Set @pnCountryUnknown	= null
	Set @pnCategoryUnknown	= null
	Set @pnSubTypeUnknown	= null
	Set @pbExactMatch	= 1
...
Begin
	...
	Set @sSQLString = 
		...
		"		@psProgramID	=	ProgramKey,"+CHAR(10)+	
		"		@psCaseType	=	CaseTypeCode,"+CHAR(10)+	
		"		@psCountryCode	=	CountryCode,"+CHAR(10)+	
		"		@pnCountryUnknown=	CountryUnknown,"+CHAR(10)+	
		"		@psPropertyType	=	PropertyTypeCode,"+CHAR(10)+
		"		@pnPropertyUnknown=	PropertyUnknown,"+CHAR(10)+	
		"		@psCaseCategory	=	CaseCategoryCode,"+CHAR(10)+	
		"		@pnCategoryUnknown=	CategoryUnknown,"+CHAR(10)+	
		"		@psSubType	=	SubTypeCode,"+CHAR(10)+	
		"		@pnSubTypeUnknown=	SubTypeUnknown,"+CHAR(10)+	
		...
		"from OPENXML(@idoc, '/ipw_ListCaseControlCriteria/FilterCriteriaGroup/FilterCriteria["+convert(nvarchar(3), @pnFilterGroupIndex)+"]',2)"+CHAR(10)+
		"	WITH ("+CHAR(10)+
		"	CaseOfficeKey		int		'CaseOffice/OfficeID/text()',"+CHAR(10)+
		"	ProgramKey		nvarchar(8)	'Program/ProgramID/text()',"+CHAR(10)+
		"	CaseTypeCode		nvarchar(1)	'CaseType/CaseTypeCode/text()',"+CHAR(10)+
		"	CountryCode		nvarchar(3)	'Country/CountryCode/text()',"+CHAR(10)+
		"	CountryUnknown		decimal(1,0)	'Unknown/CountryUnknown/text()',"+CHAR(10)+
		"	PropertyTypeCode	nvarchar(1)	'PropertyType/PropertyTypeCode/text()',"+CHAR(10)+
		"	PropertyUnknown		decimal(1,0)	'Unknown/PropertyUnknown/text()',"+CHAR(10)+
		"	CaseCategoryCode	nvarchar(2)	'CaseCategory/CaseCategoryCode/text()',"+CHAR(10)+
		"	CategoryUnknown		decimal(1,0)	'Unknown/CategoryUnknown/text()',"+CHAR(10)+
		"	SubTypeCode		nvarchar(2)	'SubType/SubTypeCode/text()',"+CHAR(10)+
		"	SubTypeUnknown		decimal(1,0)	'Unknown/SubTypeUnknown/text()',"+CHAR(10)+
		...
		exec @nErrorCode = sp_executesql @sSQLString,
			 N'	@idoc			int,
				...
				@pnCountryUnknown	bit		output,
				@psPropertyType		nvarchar(1)	output,
				@pnPropertyUnknown	bit		output,
				@psCaseCategory		nvarchar(2)	output,
				@pnCategoryUnknown	bit		output,
				@psSubType		nvarchar(2)	output,
				@pnSubTypeUnknown	bit		output,
				...
				@pnCountryUnknown	=	@pnCountryUnknown	output,
				@psPropertyType		=	@psPropertyType		output,
				@pnPropertyUnknown	=	@pnPropertyUnknown	output,
				@psCaseCategory		=	@psCaseCategory		output,
				@pnCategoryUnknown	=	@pnCategoryUnknown	output,
				@psSubType		=	@psSubType		output,
				@pnSubTypeUnknown	=	@pnSubTypeUnknown	output,
...
Begin	
	...
	Else Begin
		-- RFC34931
		-- Reset the UNKNOWN flags to NULL if they are 
		-- set to zero if the @ExactMatch option is on.
		If @pbExactMatch=1
		Begin
			If @pnPropertyUnknown=0
				set @pnPropertyUnknown=null
				
			If @pnCountryUnknown=0
				set @pnCountryUnknown=null
				
			If @pnCategoryUnknown=0
				set @pnCategoryUnknown=null
				
			If @pnSubTypeUnknown=0
				set @pnSubTypeUnknown=null
		End
		...
		Set @sFrom = "from dbo.fn_GetCriteriaRows"+char(10)+
	        ...
	        "DEFAULT, -- @pnCheckListType"+char(10)+		
	        "@psProgramID,"+char(10)+
	        ...
	        "@psSubType,"+char(10)+
	        "@psBasis,"+char(10)+
	        "DEFAULT, -- @psRegisteredUsers"+char(10)+		
	        "DEFAULT, -- @pnTypeOfMark"+char(10)+		
	        "DEFAULT, -- @pnLocalClientFlag"+char(10)+		
	        "DEFAULT, -- @pnTableCode"+char(10)+		
	        "DEFAULT, -- @pdtDateOfAct"+char(10)+		
	        "@pnRuleInUse,"+char(10)+
	        "@pnPropertyUnknown,"+char(10)+
	        "@pnCountryUnknown,"+char(10)+
	        "@pnCategoryUnknown,"+char(10)+
	        "@pnSubTypeUnknown,"+char(10)+
...
Begin
	...
	Begin
		...
		Begin
			Set @sTableColumn='PR.PROGRAMNAME'
			Set @sFrom = @sFrom +CHAR(10)+'left Join PROGRAM PR on (PR.PROGRAMID=T.PROGRAMID)'
		End
...
Begin
	set @sSQLString = @sSelect +CHAR(10)+ @sFrom + CHAR(10)+ 
				CASE WHEN @pbHasCRMLicense=0 THEN "where T.PROGRAMID not in" +char(10)+ 
				"(select COLCHARACTER from SITECONTROL where CONTROLID = 'CRM Screen Control Program')" +char(10)+
				" and T.CASETYPE not in (select CASETYPE from CASETYPE where CRMONLY = 1)"+char(10)+
				" and (T.PROPERTYTYPE not in (select PROPERTYTYPE from PROPERTYTYPE where CRMONLY = 1) or T.PROPERTYTYPE is null)" END +    
...
begin
	exec @nErrorCode=sp_executesql @sSQLString,
			...
			@pnPropertyUnknown	decimal(1,0)	OUTPUT,
			@pnCountryUnknown	decimal(1,0)	OUTPUT,
			@pnCategoryUnknown	decimal(1,0)	OUTPUT,
			@pnSubTypeUnknown	decimal(1,0)	OUTPUT,
			@pbExactMatch		bit             OUTPUT,
			@pnProfileKey		int		OUTPUT,
			@pnCriteriaNo		int',
			@pnCaseOfficeID		=	@pnCaseOfficeID		OUTPUT,
...
Grant execute on dbo.ipw_ListCaseControlCriteria to public
GO
