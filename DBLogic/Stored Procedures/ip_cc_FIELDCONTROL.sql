-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_FIELDCONTROL
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_FIELDCONTROL]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_FIELDCONTROL.'
	drop procedure dbo.ip_cc_FIELDCONTROL
	print '**** Creating procedure dbo.ip_cc_FIELDCONTROL...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_FIELDCONTROL
...
-- PROCEDURE :	ip_cc_FIELDCONTROL
-- VERSION :	1
-- DESCRIPTION:	The comparison/display and merging of imported data for the FIELDCONTROL table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_FIELDCONTROL table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_FIELDCONTROL('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	ATTRIBUTES= I.ATTRIBUTES,
			FIELDLITERAL= I.FIELDLITERAL,
			INHERITED= I.INHERITED
		from	FIELDCONTROL C
		join	CCImport_FIELDCONTROL I	on ( I.CRITERIANO=C.CRITERIANO
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_FIELDCONTROL  to public
go
