-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_DATASOURCE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_DATASOURCE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_DATASOURCE.'
	drop procedure dbo.ip_cc_DATASOURCE
	print '**** Creating procedure dbo.ip_cc_DATASOURCE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_DATASOURCE
...
-- PROCEDURE :	ip_cc_DATASOURCE
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the DATASOURCE table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_DATASOURCE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_DATASOURCE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	SYSTEMID= I.SYSTEMID,
			SOURCENAMENO= I.SOURCENAMENO,
			ISPROTECTED= I.ISPROTECTED,
			DATASOURCECODE= I.DATASOURCECODE
		from	DATASOURCE C
		join	CCImport_DATASOURCE I	on ( I.DATASOURCEID=C.DATASOURCEID)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_DATASOURCE  to public
go
