-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_VALIDCATEGORY
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_VALIDCATEGORY]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_VALIDCATEGORY.'
	drop procedure dbo.ip_cc_VALIDCATEGORY
	print '**** Creating procedure dbo.ip_cc_VALIDCATEGORY...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_VALIDCATEGORY
...
-- Prerequisite that the CCImport_VALIDCATEGORY table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_VALIDCATEGORY('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	CASECATEGORYDESC= I.CASECATEGORYDESC,
			PROPERTYEVENTNO= I.PROPERTYEVENTNO,
			MULTICLASSPROPERTYAPP= I.MULTICLASSPROPERTYAPP
		from	VALIDCATEGORY C
		join	CCImport_VALIDCATEGORY I	on ( I.COUNTRYCODE=C.COUNTRYCODE
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_VALIDCATEGORY  to public
go
