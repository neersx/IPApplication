-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_VALIDPROPERTY
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_VALIDPROPERTY]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_VALIDPROPERTY.'
	drop procedure dbo.ip_cc_VALIDPROPERTY
	print '**** Creating procedure dbo.ip_cc_VALIDPROPERTY...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_VALIDPROPERTY
...
-- Prerequisite that the CCImport_VALIDPROPERTY table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_VALIDPROPERTY('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	PROPERTYNAME= I.PROPERTYNAME,
			OFFSET= I.OFFSET,
			CYCLEOFFSET= I.CYCLEOFFSET,
			ANNUITYTYPE= I.ANNUITYTYPE
		from	VALIDPROPERTY C
		join	CCImport_VALIDPROPERTY I	on ( I.COUNTRYCODE=C.COUNTRYCODE
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_VALIDPROPERTY  to public
go
