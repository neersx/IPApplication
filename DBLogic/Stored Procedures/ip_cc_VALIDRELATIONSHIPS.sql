-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_VALIDRELATIONSHIPS
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_VALIDRELATIONSHIPS]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_VALIDRELATIONSHIPS.'
	drop procedure dbo.ip_cc_VALIDRELATIONSHIPS
	print '**** Creating procedure dbo.ip_cc_VALIDRELATIONSHIPS...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_VALIDRELATIONSHIPS
...
-- PROCEDURE :	ip_cc_VALIDRELATIONSHIPS
-- VERSION :	1
...
-- Prerequisite that the CCImport_VALIDRELATIONSHIPS table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_VALIDRELATIONSHIPS('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		join	CCImport_VALIDRELATIONSHIPS I	on ( I.COUNTRYCODE=C.COUNTRYCODE
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_VALIDRELATIONSHIPS  to public
go
