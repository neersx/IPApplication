-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_VALIDSUBTYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_VALIDSUBTYPE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_VALIDSUBTYPE.'
	drop procedure dbo.ip_cc_VALIDSUBTYPE
	print '**** Creating procedure dbo.ip_cc_VALIDSUBTYPE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_VALIDSUBTYPE
...
-- Prerequisite that the CCImport_VALIDSUBTYPE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_VALIDSUBTYPE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update VALIDSUBTYPE
		set	SUBTYPEDESC= I.SUBTYPEDESC
		from	VALIDSUBTYPE C
		join	CCImport_VALIDSUBTYPE I	on ( I.COUNTRYCODE=C.COUNTRYCODE
						and I.PROPERTYTYPE=C.PROPERTYTYPE
						and I.CASETYPE=C.CASETYPE
						and I.CASECATEGORY=C.CASECATEGORY
						and I.SUBTYPE=C.SUBTYPE)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_VALIDSUBTYPE  to public
go
