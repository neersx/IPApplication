-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_PROFILES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_PROFILES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_PROFILES.'
	drop procedure dbo.ip_cc_PROFILES
	print '**** Creating procedure dbo.ip_cc_PROFILES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_PROFILES
...
-- PROCEDURE :	ip_cc_PROFILES
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the PROFILES table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_PROFILES table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_PROFILES('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		from	PROFILES C
		join	CCImport_PROFILES I	on ( I.PROFILEID=C.PROFILEID)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_PROFILES  to public
go
