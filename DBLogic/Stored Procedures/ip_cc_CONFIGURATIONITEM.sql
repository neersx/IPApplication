-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_CONFIGURATIONITEM
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_CONFIGURATIONITEM]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_CONFIGURATIONITEM.'
	drop procedure dbo.ip_cc_CONFIGURATIONITEM
	print '**** Creating procedure dbo.ip_cc_CONFIGURATIONITEM...'
	print ''
end
go
...
CREATE PROCEDURE [dbo].[ip_cc_CONFIGURATIONITEM]
...
-- PROCEDURE :	ip_cc_CONFIGURATIONITEM
-- VERSION :	3
-- DESCRIPTION:	The comparison/display and merging of imported data for the CONFIGURATIONITEM table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_CONFIGURATIONITEM table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_CONFIGURATIONITEM('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	TASKID= I.TASKID,
			...
			GENERICPARAM= I.GENERICPARAM,
			GROUPID= I.GROUPID,
			URL= I.URL
		from	CONFIGURATIONITEM C
		join	CCImport_CONFIGURATIONITEM I	on ( I.CONFIGITEMID=C.CONFIGITEMID)
" Set @sSQLString1="
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT CONFIGURATIONITEM ON
		...
		SET IDENTITY_INSERT CONFIGURATIONITEM OFF
	End
End
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_CONFIGURATIONITEM  to public
go
