-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_INSTRUCTIONTYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_INSTRUCTIONTYPE]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_INSTRUCTIONTYPE.'
	drop procedure dbo.ip_cc_INSTRUCTIONTYPE
	print '**** Creating procedure dbo.ip_cc_INSTRUCTIONTYPE...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_INSTRUCTIONTYPE
...
-- PROCEDURE :	ip_cc_INSTRUCTIONTYPE
-- VERSION :	1
...
-- Prerequisite that the CCImport_INSTRUCTIONTYPE table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_INSTRUCTIONTYPE('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		set	NAMETYPE= I.NAMETYPE,
			INSTRTYPEDESC= I.INSTRTYPEDESC,
			RESTRICTEDBYTYPE= I.RESTRICTEDBYTYPE
		from	INSTRUCTIONTYPE C
		join	CCImport_INSTRUCTIONTYPE I	on ( I.INSTRUCTIONTYPE=C.INSTRUCTIONTYPE)
" Set @sSQLString1="
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_INSTRUCTIONTYPE  to public
go
