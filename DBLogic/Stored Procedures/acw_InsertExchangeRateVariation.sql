-----------------------------------------------------------------------------------------------------------------------------
-- Creation of acw_InsertExchangeRateVariation
-----------------------------------------------------------------------------------------------------------------------------
If exists (select * from dbo.sysobjects where id = object_id(N'[dbo].[acw_InsertExchangeRateVariation]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
Begin
	Print '**** Drop Stored Procedure dbo.acw_InsertExchangeRateVariation.'
	Drop procedure [dbo].[acw_InsertExchangeRateVariation]
End
Print '**** Creating Stored Procedure dbo.acw_InsertExchangeRateVariation...'
Print ''
GO
...
CREATE PROCEDURE dbo.acw_InsertExchangeRateVariation
(
...
as
-- PROCEDURE:	acw_InsertExchangeRateVariation
-- VERSION:	1
-- SCOPE:	Inpro.Net
...
Begin
	...
	Begin	
		Set @sSQLString = "
				Insert into  EXCHRATEVARIATION 
						(EXCHSCHEDULEID,CURRENCYCODE,CASETYPE,CASECATEGORY,PROPERTYTYPE,
						COUNTRYCODE, CASESUBTYPE, EFFECTIVEDATE, NOTES, BUYFACTOR, SELLFACTOR ,
						BUYRATE, SELLRATE)
				values (@pnExchScheduleID, @psCurrencyCode, @psCaseTypeCode, @psCaseCategoryCode,		
						@psPropertyTypeCode, @psCountryCode, @psSubTypeCode, @pdtDateEffective,
						@psNotes, @pdBuyFactor, @pdSellFactor, @pdBuyRate, @pdSellRate)"
...
Grant exec on dbo.acw_InsertExchangeRateVariation to Public
go