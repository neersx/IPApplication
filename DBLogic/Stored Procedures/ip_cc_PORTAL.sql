-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_PORTAL
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_PORTAL]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_PORTAL.'
	drop procedure dbo.ip_cc_PORTAL
	print '**** Creating procedure dbo.ip_cc_PORTAL...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_PORTAL
...
-- PROCEDURE :	ip_cc_PORTAL
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the PORTAL table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_PORTAL table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_PORTAL('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		from	PORTAL C
		join	CCImport_PORTAL I	on ( I.PORTALID=C.PORTALID)
" Set @sSQLString1="
		where 		( I.NAME <>  C.NAME)
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT PORTAL OFF
	End
End
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_PORTAL  to public
go
