-----------------------------------------------------------------------------------------------------------------------------
-- Creation of ip_cc_VALIDTABLECODES
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[ip_cc_VALIDTABLECODES]') and OBJECTPROPERTY(id, N'IsProcedure') = 1)
begin
	print '**** Drop procedure dbo.ip_cc_VALIDTABLECODES.'
	drop procedure dbo.ip_cc_VALIDTABLECODES
	print '**** Creating procedure dbo.ip_cc_VALIDTABLECODES...'
	print ''
end
go
...
CREATE PROCEDURE dbo.ip_cc_VALIDTABLECODES
...
-- PROCEDURE :	ip_cc_VALIDTABLECODES
-- VERSION :	2
-- DESCRIPTION:	The comparison/display and merging of imported data for the VALIDTABLECODES table
-- CALLED BY :	
...
-- Prerequisite that the CCImport_VALIDTABLECODES table has been loaded
...
Begin
	...
	If  @ErrorCode=0
	Begin
		Set @sSQLString="SELECT * from dbo.fn_cc_VALIDTABLECODES('"+@psUserName+"')
...
/**************** Data Update ***************************************/
	...
	Begin
		...
		Set @sSQLString="
		Update VALIDTABLECODES
		set	TABLECODE= I.TABLECODE,
			VALIDTABLECODE= I.VALIDTABLECODE,
			VALIDTABLETYPE= I.VALIDTABLETYPE
		from	VALIDTABLECODES C
		join	CCImport_VALIDTABLECODES I	on ( I.VALIDTABLECODEID=C.VALIDTABLECODEID)
" Set @sSQLString1="
...
"
	...
	Begin
		...
		SET IDENTITY_INSERT VALIDTABLECODES ON

		-- Insert the rows where existing key not found.
		Insert into VALIDTABLECODES(
			VALIDTABLECODEID,
			TABLECODE,
			VALIDTABLECODE,
			VALIDTABLETYPE)
		...
		SET IDENTITY_INSERT VALIDTABLECODES OFF
	End
End
...
RETURN @ErrorCode
go
grant execute on dbo.ip_cc_VALIDTABLECODES  to public
go
