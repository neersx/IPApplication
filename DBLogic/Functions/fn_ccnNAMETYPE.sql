-----------------------------------------------------------------------------------------------------------------------------
-- Creation of fn_ccnNAMETYPE
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[fn_ccnNAMETYPE]') and xtype='IF')
begin
	print '**** Drop function dbo.fn_ccnNAMETYPE.'
	drop function dbo.fn_ccnNAMETYPE
	print '**** Creating function dbo.fn_ccnNAMETYPE...'
	print ''
end
go
...
CREATE FUNCTION [dbo].[fn_ccnNAMETYPE]
(
@psUserName			nvarchar(40)	= 'dbo' -- the current user name which will have created the 'CCImport_' tables dbo used as default for security error instead of crash
)
RETURNS TABLE

-- FUNCTION :	fn_ccnNAMETYPE
-- VERSION :	5
...
UNION ALL 
 select	2, 'NAMETYPE', 0, 0, 0, count(*)
from CCImport_NAMETYPE I 
join NAMETYPE C	on( C.NAMETYPE=I.NAMETYPE)
where ( I.DESCRIPTION =  C.DESCRIPTION OR (I.DESCRIPTION is null and C.DESCRIPTION is null))
and ( I.PATHNAMETYPE =  C.PATHNAMETYPE OR (I.PATHNAMETYPE is null and C.PATHNAMETYPE is null))
and ( I.PATHRELATIONSHIP =  C.PATHRELATIONSHIP OR (I.PATHRELATIONSHIP is null and C.PATHRELATIONSHIP is null))
and ( I.HIERARCHYFLAG =  C.HIERARCHYFLAG OR (I.HIERARCHYFLAG is null and C.HIERARCHYFLAG is null))
and ( I.MANDATORYFLAG =  C.MANDATORYFLAG OR (I.MANDATORYFLAG is null and C.MANDATORYFLAG is null))
and ( I.KEEPSTREETFLAG =  C.KEEPSTREETFLAG OR (I.KEEPSTREETFLAG is null and C.KEEPSTREETFLAG is null))
and ( I.COLUMNFLAGS =  C.COLUMNFLAGS OR (I.COLUMNFLAGS is null and C.COLUMNFLAGS is null))
and ( I.MAXIMUMALLOWED =  C.MAXIMUMALLOWED OR (I.MAXIMUMALLOWED is null and C.MAXIMUMALLOWED is null))
and ( I.PICKLISTFLAGS =  C.PICKLISTFLAGS OR (I.PICKLISTFLAGS is null and C.PICKLISTFLAGS is null))
and ( I.SHOWNAMECODE =  C.SHOWNAMECODE OR (I.SHOWNAMECODE is null and C.SHOWNAMECODE is null))
and ( I.DEFAULTNAMENO =  C.DEFAULTNAMENO OR (I.DEFAULTNAMENO is null and C.DEFAULTNAMENO is null))
and ( I.NAMERESTRICTFLAG =  C.NAMERESTRICTFLAG OR (I.NAMERESTRICTFLAG is null and C.NAMERESTRICTFLAG is null))
and ( I.CHANGEEVENTNO =  C.CHANGEEVENTNO OR (I.CHANGEEVENTNO is null and C.CHANGEEVENTNO is null))
and ( I.FUTURENAMETYPE =  C.FUTURENAMETYPE OR (I.FUTURENAMETYPE is null and C.FUTURENAMETYPE is null))
and ( I.USEHOMENAMEREL =  C.USEHOMENAMEREL)
and ( I.UPDATEFROMPARENT =  C.UPDATEFROMPARENT)
and ( I.OLDNAMETYPE =  C.OLDNAMETYPE OR (I.OLDNAMETYPE is null and C.OLDNAMETYPE is null))
and ( I.BULKENTRYFLAG =  C.BULKENTRYFLAG OR (I.BULKENTRYFLAG is null and C.BULKENTRYFLAG is null))
...
and ( I.PRIORITYORDER   =  C.PRIORITYORDER   OR (I.PRIORITYORDER   is null and C.PRIORITYORDER   is  null))
and ( I.NATIONALITYFLAG =  C.NATIONALITYFLAG OR (I.NATIONALITYFLAG is null and C.NATIONALITYFLAG is  null))
...
go
grant DELETE, INSERT, REFERENCES, SELECT, UPDATE on dbo.fn_ccnNAMETYPE  to public
go
