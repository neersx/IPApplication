-----------------------------------------------------------------------------------------------------------------------------
-- Creation of fn_cc_PROGRAM
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[fn_cc_PROGRAM]') and xtype='IF')
begin
	print '**** Drop function dbo.fn_cc_PROGRAM.'
	drop function dbo.fn_cc_PROGRAM
	print '**** Creating function dbo.fn_cc_PROGRAM...'
	print ''
end
go

SET NOCOUNT ON
go

-- Table must exist in COPYCONFIG user space at time of function creation.
if not exists (select * from sysobjects where id = object_id(N'[CCImport_PROGRAM]') and xtype='U')
begin
	select * 
	into CCImport_PROGRAM 
	from PROGRAM
	where 1=0
	print ''
end
go


SET QUOTED_IDENTIFIER OFF 
go


CREATE FUNCTION dbo.fn_cc_PROGRAM
(
@psUserName			nvarchar(40)	= 'dbo' -- the current user name which will have created the 'CCImport_' tables dbo used as default for security error instead of crash
)
RETURNS TABLE

-- FUNCTION :	fn_cc_PROGRAM
-- VERSION :	1
-- DESCRIPTION:	The SELECT to display of imported data for the PROGRAM table
-- CALLED BY :	ip_CopyConfigPROGRAM
-- MODIFICATIONS
-- Date		Who	Number	Version	Description
-- ----         ---	------	-------	-----------------------------------------------------------
-- 21 Aug 2019	MF	DR-42774 1	Function generated
--
As 
Return
select	1 as 'Switch',
	'X' as 'Match',
	'D' as 'Imported -',
	 null as 'Imported Programid',
	 null as 'Imported Programname',
	 null as 'Imported Parentprogram',
	 null as 'Imported Programgroup',
	'D' as '-',
	 C.PROGRAMID     as 'Programid',
	 C.PROGRAMNAME   as 'Programname',
	 C.PARENTPROGRAM as 'Parentprogram',
	 C.PROGRAMGROUP  as 'Programgroup'
from CCImport_PROGRAM I 
	right join PROGRAM C on( C.PROGRAMID=I.PROGRAMID )
where I.PROGRAMID is null
UNION ALL 
select	1,
	'X',
	'I',
	 I.PROGRAMID,
	 I.PROGRAMNAME,
	 I.PARENTPROGRAM,
	 I.PROGRAMGROUP,
	'I',
	 null ,
	 null ,
	 null ,
	 null
from CCImport_PROGRAM I 
	left join PROGRAM C on( C.PROGRAMID=I.PROGRAMID)
where C.PROGRAMID is null
UNION ALL 
select	2,
	'O',
	'U',
	 I.PROGRAMID,
	 I.PROGRAMNAME,
	 I.PARENTPROGRAM,
	 I.PROGRAMGROUP,
	'U',
	 C.PROGRAMID,
	 C.PROGRAMNAME,
	 C.PARENTPROGRAM,
	 C.PROGRAMGROUP
from CCImport_PROGRAM I 
	join PROGRAM C	on ( C.PROGRAMID=I.PROGRAMID)
where 	( I.PROGRAMNAME   <>  C.PROGRAMNAME   OR (I.PROGRAMNAME   is null and C.PROGRAMNAME   is not null) OR (I.PROGRAMNAME   is not null and C.PROGRAMNAME   is null))
OR 	( I.PARENTPROGRAM <>  C.PARENTPROGRAM OR (I.PARENTPROGRAM is null and C.PARENTPROGRAM is not null) OR (I.PARENTPROGRAM is not null and C.PARENTPROGRAM is null))
OR 	( I.PROGRAMGROUP  <>  C.PROGRAMGROUP  OR (I.PROGRAMGROUP  is null and C.PROGRAMGROUP  is not null) OR (I.PROGRAMGROUP  is not null and C.PROGRAMGROUP  is null))

go

-- Remove table now that function is created.
if  exists (select * from sysobjects where id = object_id(N'[CCImport_PROGRAM]') and xtype='U')
begin
	drop table CCImport_PROGRAM 
	print ''
end
go
grant DELETE, INSERT, REFERENCES, SELECT, UPDATE on dbo.fn_cc_PROGRAM  to public
go
