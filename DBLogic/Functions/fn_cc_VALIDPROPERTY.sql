-----------------------------------------------------------------------------------------------------------------------------
-- Creation of fn_cc_VALIDPROPERTY
-----------------------------------------------------------------------------------------------------------------------------
if exists (select * from sysobjects where id = object_id(N'[dbo].[fn_cc_VALIDPROPERTY]') and xtype='IF')
begin
	print '**** Drop function dbo.fn_cc_VALIDPROPERTY.'
	drop function dbo.fn_cc_VALIDPROPERTY
	print '**** Creating function dbo.fn_cc_VALIDPROPERTY...'
	print ''
end
go

SET NOCOUNT ON
go

-- Table must exist in COPYCONFIG user space at time of function creation.
if not exists (select * from sysobjects where id = object_id(N'[CCImport_VALIDPROPERTY]') and xtype='U')
begin
	select * 
	into CCImport_VALIDPROPERTY 
	from VALIDPROPERTY
	where 1=0
	print ''
end
go


SET QUOTED_IDENTIFIER OFF 
go


CREATE FUNCTION dbo.fn_cc_VALIDPROPERTY
(
@psUserName			nvarchar(40)	= 'dbo' -- the current user name which will have created the 'CCImport_' tables dbo used as default for security error instead of crash
)
RETURNS TABLE

-- FUNCTION :	fn_cc_VALIDPROPERTY
-- VERSION :	1
-- DESCRIPTION:	The SELECT to display of imported data for the VALIDPROPERTY table
-- CALLED BY :	ip_CopyConfigVALIDPROPERTY
-- MODIFICATIONS
-- Date		Who	Number	Version	Description
-- ----         ---	------	-------	-----------------------------------------------------------
-- 20 Feb 2013	System		1	Function generated
--
As 
Return
select	1 as 'Switch',
	'X' as 'Match',
	'D' as 'Imported -',
	 null as 'Imported Countrycode',
	 null as 'Imported Propertytype',
	 null as 'Imported Propertyname',
	 null as 'Imported Offset',
	 null as 'Imported Cycleoffset',
	 null as 'Imported Annuitytype',
'D' as '-',
	 C.COUNTRYCODE as 'Countrycode',
	 C.PROPERTYTYPE as 'Propertytype',
	 C.PROPERTYNAME as 'Propertyname',
	 C.OFFSET as 'Offset',
	 C.CYCLEOFFSET as 'Cycleoffset',
	 C.ANNUITYTYPE as 'Annuitytype'
from CCImport_VALIDPROPERTY I 
	right join VALIDPROPERTY C on( C.COUNTRYCODE=I.COUNTRYCODE
and  C.PROPERTYTYPE=I.PROPERTYTYPE)
where I.COUNTRYCODE is null
UNION ALL 
select	1,
	'X',
	'I',
	 I.COUNTRYCODE,
	 I.PROPERTYTYPE,
	 I.PROPERTYNAME,
	 I.OFFSET,
	 I.CYCLEOFFSET,
	 I.ANNUITYTYPE,
'I',
	 null ,
	 null ,
	 null ,
	 null ,
	 null ,
	 null
from CCImport_VALIDPROPERTY I 
	left join VALIDPROPERTY C on( C.COUNTRYCODE=I.COUNTRYCODE
and  C.PROPERTYTYPE=I.PROPERTYTYPE)
where C.COUNTRYCODE is null
UNION ALL 
select	2,
	'O',
	'U',
	 I.COUNTRYCODE,
	 I.PROPERTYTYPE,
	 I.PROPERTYNAME,
	 I.OFFSET,
	 I.CYCLEOFFSET,
	 I.ANNUITYTYPE,
'U',
	 C.COUNTRYCODE,
	 C.PROPERTYTYPE,
	 C.PROPERTYNAME,
	 C.OFFSET,
	 C.CYCLEOFFSET,
	 C.ANNUITYTYPE
from CCImport_VALIDPROPERTY I 
	join VALIDPROPERTY C	on ( C.COUNTRYCODE=I.COUNTRYCODE
	and C.PROPERTYTYPE=I.PROPERTYTYPE)
where 	( I.PROPERTYNAME <>  C.PROPERTYNAME OR (I.PROPERTYNAME is null and C.PROPERTYNAME is not null) 
OR (I.PROPERTYNAME is not null and C.PROPERTYNAME is null))
	OR 	( I.OFFSET <>  C.OFFSET OR (I.OFFSET is null and C.OFFSET is not null) 
OR (I.OFFSET is not null and C.OFFSET is null))
	OR 	( I.CYCLEOFFSET <>  C.CYCLEOFFSET OR (I.CYCLEOFFSET is null and C.CYCLEOFFSET is not null) 
OR (I.CYCLEOFFSET is not null and C.CYCLEOFFSET is null))
	OR 	( I.ANNUITYTYPE <>  C.ANNUITYTYPE OR (I.ANNUITYTYPE is null and C.ANNUITYTYPE is not null) 
OR (I.ANNUITYTYPE is not null and C.ANNUITYTYPE is null))

go

-- Remove table now that function is created.
if  exists (select * from sysobjects where id = object_id(N'[CCImport_VALIDPROPERTY]') and xtype='U')
begin
	drop table CCImport_VALIDPROPERTY 
	print ''
end
go
grant DELETE, INSERT, REFERENCES, SELECT, UPDATE on dbo.fn_cc_VALIDPROPERTY  to public
go
