...
namespace Inprotech.IntegrationServer.PtoAccess.Epo.Activities
{
    public class DueSchedule
    {
        ...
        public async Task<Activity> Execute(int scheduleId, Guid cancellationToken)
        {
            ...
            var epoSchedule = schedule.GetExtendedSettings<EpoSchedule>();

            var sessionId = _scheduleRuntimeEvents.StartSchedule(schedule, cancellationToken, null, null);
            ...
            var runWorkflow =
                Activity.Run<ResolveEligibleCases>(
                    _ => _.From(dataDownloadSession, epoSchedule.SavedQueryId, epoSchedule.RunAsUserId));
            ...
            var validate = BuildValidationActivities(epoSchedule);
        ...
        static Activity BuildValidationActivities(EpoSchedule epoSchedule)
        {
            var validateRequiredSettings = Activity.Run<EnsureScheduleValid>(e => e.ValidateRequiredSettings(epoSchedule));
