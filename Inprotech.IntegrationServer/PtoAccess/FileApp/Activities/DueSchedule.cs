...
namespace Inprotech.IntegrationServer.PtoAccess.FileApp.Activities
{
    public class DueSchedule
    {
        ...
        public async Task<Activity> Execute(int scheduleId, Guid cancellationToken)
        {
            ...
            var fileAppSchedule = schedule.GetExtendedSettings<FileAppSchedule>();
            ...
            var runWorkflow =
                Activity.Run<ResolveEligibleCases>(_ => _.From(dataDownloadSession, fileAppSchedule.SavedQueryId, fileAppSchedule.RunAsUserId));
            ...
            var validate = BuildValidationActivities(fileAppSchedule);
            ...
            return workflow
                .Cancelled(Activity.Run<Innography.Activities.DueSchedule>(d => d.CancelRun(dataDownloadSession)))
        ...
        static Activity BuildValidationActivities(FileAppSchedule fileAppSchedule)
        {
            var validateRequiredSettings = Activity.Run<EnsureScheduleValid>(e => e.ValidateRequiredSettings(fileAppSchedule));
