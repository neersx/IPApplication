...
namespace Inprotech.Tests.Integration.Schedules.Extensions.Innography
{
    public class CreateScheduleFacts : FactBase
    {
        ...
        public async Task CreatesSchedule()
        {
            ...
            var e = JsonConvert.DeserializeObject<InnographySchedule>(r.Schedule.ExtendedSettings);
        ...
        {
            var subject = new CreateSchedule(ConfigurePrerequisite("missing-platform-registration"));
            var r = await subject.TryCreateFrom(Build(QueryKey, RunAs));
            Assert.False(r.IsValid);
            Assert.Equal("missing-platform-registration", r.ValidationResult);
        }
    }
}