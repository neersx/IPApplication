    ...
    public class DownloadCaseDispatcherFacts
    {
        public class DispatchMethod : FactBase
        {
            ...
            public async Task CasesAreCreated()
            {
                ...
                await fixture.Subject.Dispatch(new[] {a, b, c});
            ...
            public async Task SavesTheListThenDispatch()
            {
                ...
                fixture.DataDownloadLocationResolver
                       .ResolveRoot(a, "applicationlist.json")
                       .Returns(Path.Combine("abcdef", "applicationlist.json"));

                var r = await fixture.Subject.Dispatch(new[] {a, b, c});

                Assert.Equal("DownloadRequired.FromTheIPPlatform", ((SingleActivity) r).TypeAndMethod());

                Assert.Equal(Path.Combine("abcdef", generatedFileName), ((SingleActivity) r).Arguments[0]);

                Assert.Equal(Path.Combine("abcdef", "applicationlist.json"), ((SingleActivity) r).Arguments[1]);
