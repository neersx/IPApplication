...
{
    public class IfThereAreDataValidationErrorsInTheUpdate : FactBase
    {
        [Fact]
        public async Task It_should_return_the_results_of_the_update_with_data_validation_errors()
        {
            ...
            singleCaseUpdateFixture.ExistingDispatcherResults = new DataEntryTaskCompletionResult(
                                                                                                  ...
                                                                                                  new[]
                                                                                                  {
                                                                                                      new ValidationResult(Fixture.String("Message")).CorrelateWithEntity(singleCaseUpdateFixture.ExistingDataEntryTask)
                                                                                                                                                     .WithDetails(new {ValidationKey = Fixture.Integer()})
                                                                                                  });
